...
function getPreferenceFilePath() {
  return join(getDataDir(), "geo-region-preferences.json");
}
 ...
 *
 * The expected format is comma-separated entries of the form:
 *   "<regionId>:<lat>,<lon>:<url>"
 *
 * Both the lat,lon coordinate pair and the entry separator use commas, so
 * this parser walks the string and splits at commas that immediately precede
 * a "<regionId>:" boundary (a word followed by a colon).
 *
 * @param {string} [raw]
 * @returns {Region[]}
 */
export function parseRegionsConfig(raw) {
  ...
  // Split on commas that introduce a new "<id>:" entry. Negative lookahead for
  // a digit prevents splitting inside "lat,lon".
  const entries = value.split(/,(?=[A-Za-z][\w-]*:)/g)
    .map((s) => s.trim())
    .filter(Boolean);
...
/**
 * Set the preferred region for a user. Stored on disk so it survives restarts.
 * @param {string} userId
...
export async function setUserRegionPreference(userId, regionId) {
  ...
  const result = await withPathLock(path, async () => {
    const data = await readJsonPath(path, { _version: 1, prefs: {} });
    if (!data.prefs || typeof data.prefs !== "object") data.prefs = {};
    if (regionId === null || regionId === undefined || regionId === "") {
      delete data.prefs[String(userId)];
      await writeJsonPath(path, data);
      return { userId: String(userId), regionId: null };
    }
    data.prefs[String(userId)] = {
      regionId: String(regionId),
      updatedAt: new Date().toISOString(),
    };
    await writeJsonPath(path, data);
    return { userId: String(userId), regionId: String(regionId) };
  });
  return result;
}
...
export async function getUserRegionPreference(userId) {
  ...
  try {
    const data = await readJsonPath(getPreferenceFilePath(), { prefs: {} });
    const entry = data && data.prefs && data.prefs[String(userId)];
    if (!entry) return null;
 ...
 * @returns {Promise<Array<{ userId: string, regionId: string|null, updatedAt: string|null }>>}
 */
export async function listUserRegionPreferences() {
  try {
    const data = await readJsonPath(getPreferenceFilePath(), { prefs: {} });
    const prefs = (data && data.prefs) || {};
    return Object.entries(prefs).map(([userId, entry]) => ({
      userId,
