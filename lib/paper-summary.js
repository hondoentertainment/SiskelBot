...
export function extractSections(text) {
  ...
  for (const line of lines) {
    ...
    // A line is a heading if it is short and equals a known heading (with optional trailing ":").
    const m = trimmed.length <= 40
      ? headings.find((h) => lower === h || lower === `${h}:`)
      : null;
    if (m) {
      current = m === "method" ? "methods" : m;
