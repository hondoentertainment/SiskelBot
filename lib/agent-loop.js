...
export async function runAgentLoop(req, res, config, model, streamOptions, backendFetch) {
  ...
  // Optional subagent budget (lib/subagent-budget.js) — callers may pass a
  // budget object via agentOptions to cap iterations/tool calls/cost/wall time.
  const subBudget =
    agentOpts.budget && typeof agentOpts.budget.check === "function" && typeof agentOpts.budget.recordIteration === "function"
      ? agentOpts.budget
      : null;
  ...
  // Wave-2 upfront planning (AGENT_UPFRONT_PLAN=1): decompose the task into a
  // plan system message before the loop. No-op by default; never throws.
  if (!hc) {
    try {
      const { runUpfrontPlanHook } = await import("./agent-loop-hooks.js");
      const lastUser = [...messages].reverse().find((m) => m?.role === "user");
  ...
  while (!skipAgentWhile && iteration < maxAgentIterations) {
    ...
    if (subBudget) {
      subBudget.recordIteration();
      const bc = subBudget.check();
      if (bc.exceeded) {
        lastContent = `(Agent stopped: budget exceeded — ${bc.reason})`;
        stopReason = "budget_exceeded";
        setAgentHeader("X-Agent-Stopped", "budget_exceeded");
        break;
      }
    }
        ...
        await agentTracer.startActiveSpan(
          ...
          async (roundSpan) => {
            try {
              ...
              if (!response.ok) {
                const err = typeof response.text === "function" ? await response.text().catch(() => "") : "";
    ...
    if (subBudget && typeof subBudget.recordToolCall === "function") {
      for (let i = 0; i < toolCalls.length; i++) subBudget.recordToolCall();
    }
    if (MAX_AGENT_TOOL_CALLS_ENV > 0 && totalToolCalls >= MAX_AGENT_TOOL_CALLS_ENV) {
      lastContent = "(Agent stopped: tool call budget reached; partial context is in the conversation.)";
      stopReason = "tool_budget";
  ...
  return {
    ...
    stopReason,
    stopDetail,
    ...(subBudget && typeof subBudget.summary === "function" ? { budgetSummary: subBudget.summary() } : {}),
