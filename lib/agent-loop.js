  ...
  while (iteration < maxAgentIterations) {
    ...
    if (replanEnabled) {
      for (const r of results) {
        ...
        const errorSummary = typeof parsed.error === "string"
          ? parsed.error.slice(0, 300)
          : typeof parsed.message === "string"
            ? parsed.message.slice(0, 300)
            : Array.isArray(parsed.errors)
              ? parsed.errors.map(String).join("; ").slice(0, 300)
              : "unknown error";
        ...
        if (replanCount <= AGENT_MAX_REPLANS) {
          messages.push({
            role: "system",
            content: `Tool '${toolName}' failed with: ${errorSummary}. Re-evaluate your approach. Consider alternative tools or a different strategy. Do not retry the same tool with the same arguments.`,
          });
          if (liveSessionId) {
            try {
              publishAgentRunEvent(liveSessionId, "status.change", {
