...
import { matchProcedures, learnProcedureFromRun } from "./procedural-memory.js";
import { findMemoryConflicts, listMemories } from "./agent-memory.js";
import {
  getAdviceModeStatus,
  resurfaceThoughts,
  formatThoughtsHint,
...
export async function runAgentLoop(req, res, config, model, streamOptions, backendFetch) {
  ...
  if (!resumeAfterHitl) {
    ...
    if (process.env.AGENT_MEMORY_INJECT !== "0") {
      try {
        ...
        messages = injected.messages || messages;
        if (userMessage && process.env.AGENT_PROCEDURAL_HINT !== "0") {
          const procs = await matchProcedures(workspace, userMessage);
          if (procs.length > 0) {
            const lines = procs.slice(0, 3).map((p, i) => {
              const steps = (p.steps || []).map((s) => s.tool).join(" → ");
              return `${i + 1}. ${p.goal}: ${steps}`;
            });
            messages.push({
              role: "system",
              content: `Known successful procedures for similar goals:\n${lines.join("\n")}`,
            });
          }
        }
        if (process.env.AGENT_MEMORY_CONFLICT_HINT !== "0") {
          const { memories } = await listMemories(storageUserId, workspace, { limit: 200 });
          const conflicts = findMemoryConflicts(memories || []);
          if (conflicts.length > 0) {
            messages.push({
              role: "system",
              content:
                `Memory conflicts detected (${conflicts.length}). Prefer the more recent/higher-importance fact and ask the user to resolve via /memory-editor.html if needed. Example: "${conflicts[0].a.content}" vs "${conflicts[0].b.content}".`,
            });
          }
        }
        if (process.env.ADVICE_THOUGHT_HINT !== "0") {
          try {
            const adviceStatus = await getAdviceModeStatus(storageUserId, workspace);
            if (adviceStatus.enabled) {
              const { thoughts } = await resurfaceThoughts(
                req.userId || storageUserId,
                workspace,
                { limit: 6, query: userMessage },
              );
              const hint = formatThoughtsHint(thoughts);
              if (hint) messages.push({ role: "system", content: hint });
            }
          } catch { /* advice thought hint is best-effort */ }
        }
  ...
  if (!resumeAfterHitl && !hc) {
    let reusedSessionPlan = false;
    if (sessionId && process.env.AGENT_REUSE_SESSION_PLAN !== "0") {
      try {
        const sess = await getAgentSession(sessionId);
        const summary = typeof sess?.planSummary === "string" ? sess.planSummary.trim() : "";
        const hasPlanMsg = messages.some(
          (m) =>
            m?.role === "system" &&
            typeof m.content === "string" &&
            m.content.includes("## Task Plan"),
        );
        if (summary && !hasPlanMsg) {
          messages.push({ role: "system", content: summary });
          reusedSessionPlan = true;
          trajCollector.record({ type: "plan_reuse", source: "session" });
          if (sess.planDag && typeof sess.planDag === "object") {
            try {
              await saveCheckpoint(sessionId, {
                ...
                timestamp: Date.now(),
                upfrontPlanDag: sess.planDag,
              });
            } catch { /* checkpoint best-effort */ }
          }
        }
      } catch { /* session plan reuse is best-effort */ }
    }
    if (!reusedSessionPlan) {
      ...
      const planResult = await runUpfrontPlanHook({
        messages,
      ...
      if (sessionId && planResult?.planDag) {
        try {
          await saveCheckpoint(sessionId, {
            sessionId,
            iteration: 0,
            ...
            model: bodyBase.model,
            timestamp: Date.now(),
            upfrontPlanDag: planResult.planDag,
          });
        } catch { /* checkpoint best-effort */ }
      }
    }
  ...
  if (stopReason === "complete" && process.env.AGENT_LEARN_PROCEDURES !== "0") {
    try {
      const lastUser = [...messages].reverse().find((m) => m?.role === "user");
      await learnProcedureFromRun(workspace, {
        goal: typeof lastUser?.content === "string" ? lastUser.content : "",
        toolCalls: toolCallsLog,
        stopReason,
      });
    } catch { /* procedural learn is best-effort */ }
  }
