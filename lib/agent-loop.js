...
import { resolvePatchContentForRun } from "./agent-prompt-canary.js";
import { recordToolFailure, recordToolOutcome } from "./metrics.js";
...
export async function runAgentLoop(req, res, config, model, streamOptions, backendFetch) {
  ...
  // AGENT_PROMPT_PATCH env var (default enabled). When a canary deployment
  // is active for this workspace, resolvePatchContentForRun routes a fixed
  // fraction of traffic (by hash(runId+workspace)) to the candidate patch.
  let canaryArm = "none";
  if (process.env.AGENT_PROMPT_PATCH !== "0") {
    try {
      const resolved = await resolvePatchContentForRun(workspace, runId);
      if (resolved && resolved.content) {
        messages.push({ role: "system", content: resolved.content });
      }
      canaryArm = resolved?.arm || "none";
    } catch { /* prompt patch is best-effort */ }
  }
  setAgentHeader("X-Agent-Canary-Arm", canaryArm);
  ...
  while (iteration < maxAgentIterations) {
    ...
    if (replanEnabled) {
      for (const r of results) {
        ...
        if (replanCount <= AGENT_MAX_REPLANS) {
          // Genealogy-driven alternative recommendation. Compute up to 3 tools
          // that have outperformed the failed one in prior runs of this
          // workspace; appended to the replan nudge by analyzeAndBuildReplan.
          let alternatives = [];
          if (recommenderEnabled()) {
            try {
              const lastUser = [...messages].reverse().find((m) => m?.role === "user");
              const taskText = typeof lastUser?.content === "string" ? lastUser.content : "";
              const availableToolNames = tools.map((t) => t?.function?.name).filter((n) => typeof n === "string" && n.length > 0);
              ...
              alternatives = recs.map((r) => r.tool);
    ...
    if (stagnationDetectionEnabled()) {
      ...
      const stag = stagnationTracker.check();
      if (stag.stagnant) {
        if (stagnationRecoveryEnabled()) {
          messages.push({
            role: "user",
            content: `[System] Stagnation detected (${stag.reason}). You are repeating the same tool calls. Try a different approach, use different arguments, or provide a final answer.`,
          });
        } else {
          lastContent = `(Agent stopped: ${stag.reason || "repeated tool calls with no progress"})`;
