...
export async function runAgentLoop(req, res, config, model, streamOptions, backendFetch) {
  ...
  // TODO: if no sessionId is supplied, we cannot resolve one from runId alone
  // without a reverse index. For now, events only flow when the caller passes
  // agentOptions.sessionId (or streamOptions.sessionId). See agent-session.js.
  const liveSessionId =
