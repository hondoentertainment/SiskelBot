...
export async function applyConstitution(output, constitution, options = {}) {
  ...
  // Phase 51.4: mirror the final decision into the unified audit trail.
  try {
    const action = blocked ? "block" : revisions.length > 0 ? "revise" : "allow";
    await _recordPolicyDecision({
      workspaceId: options.workspaceId || "default",
      policyId: options.policyId || "constitutional-default",
      source: "constitutional-ai",
      action,
      reason: lastViolations.length > 0
        ? `rules=${lastViolations.map((v) => v.ruleId).join(",")}`
        : null,
      severity: lastViolations.length,
      userId: options.userId || null,
      metadata: { iterations, revisionCount: revisions.length },
    });
  } catch (_) { /* audit failures must not affect safety pipeline */ }
