...
const RAG_RELEVANCE_THRESHOLD = Number(process.env.RAG_RELEVANCE_THRESHOLD) || 0.7;
...
export async function semanticSearch(opts) {
  ...
  recordRagRetrievalDuration(Date.now() - t0);

  const snippets = scored.slice(0, Math.min(topK, MAX_SNIPPETS));

  const isRelevant = (s) => s.score >= RAG_RELEVANCE_THRESHOLD;
  const precisionAtK = (k) => {
    const slice = snippets.slice(0, k);
    if (slice.length === 0) return 0;
    return slice.filter(isRelevant).length / slice.length;
  };
  recordRagPrecision({ p1: precisionAtK(1), p3: precisionAtK(3), p5: precisionAtK(5) });

  let mrr = 0;
  for (let i = 0; i < snippets.length; i++) {
    if (isRelevant(snippets[i])) { mrr = 1 / (i + 1); break; }
  }
  recordRagMrr(mrr);
  recordRagChunksRetrieved(workspace, snippets.length);
