 ...
 * Phase 17: Added strategy selection (fixed, sentence, paragraph, heading).
...
const VALID_STRATEGIES = ["fixed", "sentence", "paragraph", "heading"];

/** @typedef {{ chunkSize: number, chunkOverlap: number, maxChunks: number, strategy: 'fixed'|'sentence'|'paragraph'|'heading', metadataExtraction: boolean, preserveStructure: boolean }} ChunkingConfig */
...
export const DEFAULT_CHUNKING_CONFIG = {
  ...
  maxChunks: 50,            // max chunks per document
  strategy: "fixed",        // chunking strategy: fixed | sentence | paragraph | heading
...
function normalizeConfig(raw) {
  ...
  const strategy = typeof c.strategy === "string" && VALID_STRATEGIES.includes(c.strategy)
    ? c.strategy
  ...
  return {
    ...
    maxChunks: Math.min(500, Math.max(1, Number(c.maxChunks) || DEFAULT_CHUNKING_CONFIG.maxChunks)),
    strategy,
...
/**
 * Convert token-based chunking config to character-based params for chunking functions.
 * Approximation: 1 token ~ 4 characters.
 * @param {ChunkingConfig} config
 * @returns {{ maxChars: number, overlap: number, maxChunks: number, strategy: string }}
 */
export function chunkingConfigToCharParams(config) {
  ...
  return {
    ...
    maxChunks: config.maxChunks || DEFAULT_CHUNKING_CONFIG.maxChunks,
    strategy: config.strategy || DEFAULT_CHUNKING_CONFIG.strategy,
  };
}
