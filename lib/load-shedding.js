...
/** Lower ceilings for staging / partner burn-in (set LOAD_SHEDDING_PROFILE=staging). */
...
export function resolveInitialSheddingConfig() {
  const profile = (process.env.LOAD_SHEDDING_PROFILE || "").trim().toLowerCase();
  if (profile === "staging") return STAGING_CONFIG;
...
let config = cloneConfig(resolveInitialSheddingConfig());
...
export async function loadPersistedConfig() {
  ...
  config = cloneConfig({ ...resolveInitialSheddingConfig(), ...data });
  return cloneConfig(config);
}
