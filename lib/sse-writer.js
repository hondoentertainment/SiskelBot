...
export function createSSEWriter(res, options) {
  ...
  const rawInterval = opts.flushIntervalMs;
  const flushIntervalMs = rawInterval == null || Number.isNaN(Number(rawInterval))
    ? DEFAULT_FLUSH_INTERVAL_MS
    : Math.max(0, Number(rawInterval));
  const rawMaxBatch = opts.maxBatchBytes;
  const maxBatchBytes = rawMaxBatch == null || Number.isNaN(Number(rawMaxBatch))
    ? 8192
    : Math.max(64, Number(rawMaxBatch));
