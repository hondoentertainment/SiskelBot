...
export async function recordUsage(entry) {
  ...
  if (entry.feature != null && String(entry.feature).trim()) rec.feature = String(entry.feature).trim();
  if (Number(entry.calls) > 0) rec.calls = Math.max(0, Number(entry.calls) || 0);
