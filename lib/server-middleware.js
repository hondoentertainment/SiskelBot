...
/**
 * Install the cross-cutting HTTP middleware stack on the Express app.
 *
 * This is a behavior-preserving extraction from server.js: the ordering of
 * middleware and the env-var semantics are identical to the inline wiring.
 * Returns the resolved OAuth/SSO provider descriptors so server.js can expose
 * them to route modules via `deps`.
 *
 * @param {import('express').Express} app
 * @param {object} opts
 * @param {boolean} opts.IS_PRODUCTION
 * @returns {Promise<{oauthProviders: object, ssoProviders: object}>}
 */
export async function installServerMiddleware(app, { IS_PRODUCTION }) {
  ...
  return { oauthProviders, ssoProviders };
}
