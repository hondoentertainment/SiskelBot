...
export async function fetchTextFromAllowedUrl(urlString, opts = {}) {
  ...
  return fetchTextFromUrlWithEntries(urlString, entries, opts);
}

/**
 * Same as fetchTextFromAllowedUrl but with an explicit allowlist (Phase 85 agent fetch).
 * @param {string} urlString
 * @param {string[]} entries
 * @param {object} [opts]
 */
export async function fetchTextFromUrlWithEntries(urlString, entries, opts = {}) {
  if (!entries?.length) {
    return { error: "Allowlist is empty", code: "ALLOWLIST_REQUIRED" };
  }
  ...
  for (let hop = 0; hop < 8; hop++) {
    if (!isUrlAllowedForKnowledge(current, entries)) {
      return { error: "URL not in allowlist", code: "URL_NOT_ALLOWED" };
    }
