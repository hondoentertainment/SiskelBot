...
/**
 * Evaluate shadow-traffic quality against thresholds (Phase 63 alerting).
 *
 * @param {object} opts
 * @param {string} opts.workspaceId
 * @param {number} [opts.sinceMs]
 * @param {number} [opts.minAvgJaccard]
 * @param {number} [opts.minIdenticalRatio]
...
export async function evaluateQualityAlerts({
  ...
  const ws = sanitizeId(workspaceId);
  const windowStart = Number.isFinite(sinceMs) ? sinceMs : Date.now() - 7 * DAY_MS;
  const floorJaccard = minAvgJaccard ?? Number(process.env.EVAL_IN_PROD_MIN_JACCARD ?? 0.7);
  const floorIdentical = minIdenticalRatio ?? Number(process.env.EVAL_IN_PROD_MIN_IDENTICAL_RATIO ?? 0.5);
  const floorSamples = minSamples ?? Number(process.env.EVAL_IN_PROD_MIN_SAMPLES ?? 5);

  const q = await querySamples({ workspaceId: ws, limit: 10_000 });
  const recent = (q.samples || []).filter((s) => s.timestamp >= windowStart);
  const stats = await getShadowStats({ workspaceId: ws });
  const alerts = [];

  if (recent.length >= floorSamples) {
    let jSum = 0;
    let identical = 0;
    for (const s of recent) {
      const j = s.diff?.jaccard;
      if (typeof j === "number") jSum += j;
      if (s.diff?.identical) identical += 1;
    }
    const avgJaccard = jSum / recent.length;
    const identicalRatio = identical / recent.length;
    const windowStats = {
      total: recent.length,
      avgJaccard: Number(avgJaccard.toFixed(4)),
      identicalRatio: Number(identicalRatio.toFixed(4)),
    };
    if (avgJaccard < floorJaccard) {
      alerts.push({
        type: "low_jaccard",
        severity: "warning",
        message: `avgJaccard ${windowStats.avgJaccard} below ${floorJaccard}`,
        threshold: floorJaccard,
        value: windowStats.avgJaccard,
      });
    }
    if (identicalRatio < floorIdentical) {
      alerts.push({
        type: "low_identical_ratio",
        severity: "warning",
        message: `identicalRatio ${windowStats.identicalRatio} below ${floorIdentical}`,
        threshold: floorIdentical,
        value: windowStats.identicalRatio,
      });
    }
    return {
      workspaceId: ws,
  ...
  return {
    ...
    alerts,
    firing: false,
    note: recent.length < floorSamples ? `insufficient samples (need ${floorSamples})` : null,
  };
}

/** POST alert payload to EVAL_IN_PROD_ALERT_WEBHOOK when thresholds fire. */
export async function dispatchQualityAlerts(opts = {}) {
  const result = await evaluateQualityAlerts(opts);
  const webhook = (process.env.EVAL_IN_PROD_ALERT_WEBHOOK || "").trim();
  if (result.firing && webhook) {
    try {
      await fetch(webhook, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ source: "siskelbot-eval-in-prod", ...result }),
      });
    } catch (err) {
      result.webhookError = err.message;
    }
  }
  return result;
}
