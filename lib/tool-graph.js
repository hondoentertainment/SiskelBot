...
export function buildPipeline(goal, availableTools) {
  ...
  // Use strict typeMatches for the goal so BFS does not short-circuit on
  // every loose coercion. Edge traversal still relies on the graph's
  // pre-inferred compatible edges.
  let best = null;
  for (const source of sources) {
    ...
    while (queue.length > 0) {
      ...
      if (def && typeMatches(def.outputs, goalType)) {
