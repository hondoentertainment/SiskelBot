...
const MAX_BROWSER_ALLOWED_HOSTS = Math.min(48, Math.max(1, Number(process.env.WORKSPACE_AGENT_BROWSER_HOSTS_MAX) || 32));
...
export function normalizeWorkspaceAgentSettings(raw) {
  ...
  let browserAllowedHosts = [];
  if (apRaw && Array.isArray(apRaw.browserAllowedHosts)) {
    browserAllowedHosts = apRaw.browserAllowedHosts
      .map((s) => (typeof s === "string" ? s.trim() : ""))
      .filter(Boolean)
      .slice(0, MAX_BROWSER_ALLOWED_HOSTS);
  }
  let transientToolRetryLimit = null;
  if (apRaw && apRaw.transientToolRetryLimit !== undefined && apRaw.transientToolRetryLimit !== null) {
    transientToolRetryLimit = capPositive(apRaw.transientToolRetryLimit, 0, 5);
  }
  const agentPolicy = {
