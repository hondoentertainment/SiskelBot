...
import { aggregateFeedback, listFeedback } from "./agent-feedback-store.js";
...
const MIN_FEEDBACK_SIGNAL_LINE = 10;
const MIN_COMPLAINT_TAG_COUNT = 3;
const MIN_TOOL_MENTIONS_DOWN = 3;
 ...
 * @property {number} feedbackTotal
 * @property {number | null} feedbackUpRate
 * @property {string[]} topComplaintTags
 * @property {string[]} toolsFlaggedByUsers
 */
...
export async function synthesizePatch(workspace) {
  ...
  // Feedback is best-effort — missing store or errors degrade to empty data.
  let feedbackAgg = { total: 0, upCount: 0, downCount: 0, upRate: 0, avgScore: null, tagCounts: {}, byDay: [] };
  let downFeedback;
  try {
    feedbackAgg = await aggregateFeedback(ws);
  } catch {
    /* swallow — feedback is optional signal */
  }
  try {
    downFeedback = await listFeedback(ws, { limit: 200, rating: "down" });
  } catch {
    downFeedback = [];
  }
  ...
  const signalOk =
    ...
    || cooldowns.length > 0
    || (feedbackAgg && feedbackAgg.total >= MIN_FEEDBACK);
  ...
  // Feedback-derived tools: scan down-rated feedback comments + agentResponse
  // snippets for tool-name substring matches. Substring matching is a
  // heuristic: it catches "the web_search tool was wrong" but may false-match
  // on tool names that happen to be common English words. We only count a tool
  // once per feedback event regardless of how many times it appears.
  const knownToolNames = new Set([
    ...Object.keys(genealogy.tools || {}),
    ...Object.keys(failureStats.tools || {}),
  ]);
  /** @type {Map<string, number>} */
  const userToolMentionCounts = new Map();
  for (const ev of downFeedback) {
    if (!ev || typeof ev !== "object") continue;
    const corpus = `${ev.comment || ""}\n${ev.agentResponse || ""}`;
    if (!corpus) continue;
    for (const tool of knownToolNames) {
      if (!tool) continue;
      if (corpus.includes(tool)) {
        userToolMentionCounts.set(tool, (userToolMentionCounts.get(tool) || 0) + 1);
      }
    }
  }
  const toolsFlaggedByUsers = [...userToolMentionCounts.entries()]
    .filter(([, n]) => n >= MIN_TOOL_MENTIONS_DOWN)
    .map(([tool]) => tool)
    .sort();

  // Merge user-flagged tools into the existing flagged set (feedback reason).
  for (const tool of toolsFlaggedByUsers) {
    if (!flagged.has(tool)) flagged.set(tool, "user_feedback");
  }
  ...
  // Feedback aggregate metrics.
  const feedbackTotal = Number.isFinite(feedbackAgg?.total) ? feedbackAgg.total : 0;
  const feedbackUpRate = feedbackTotal > 0 ? feedbackAgg.upRate : null;
  const upRatePct = feedbackUpRate == null ? null : Math.round(feedbackUpRate * 100);

  // Top-3 complaint tags (only counted when >= MIN_COMPLAINT_TAG_COUNT).
  const sortedTagEntries = Object.entries(feedbackAgg?.tagCounts || {})
    .filter(([, n]) => Number.isFinite(n) && n >= MIN_COMPLAINT_TAG_COUNT)
    .sort((a, b) => b[1] - a[1] || (a[0] < b[0] ? -1 : a[0] > b[0] ? 1 : 0))
    .slice(0, 3);
  const topComplaintTags = sortedTagEntries.map(([tag, n]) => `${tag}(${n})`);

  // Build sections as keyed objects so we can drop by priority under the cap.
  /** @type {Array<{key: string, text: string}>} */
  const namedSections = [];

  // Feedback signal line — most recent human truth, highest priority to keep.
  if (feedbackTotal >= MIN_FEEDBACK_SIGNAL_LINE && upRatePct != null) {
    const guidance = upRatePct >= 70
      ? "Prioritize responses that match the style users have rewarded"
      : upRatePct >= 50
        ? "Prioritize responses that are more accurate and cautious than recent output"
        : "Prioritize responses that directly address the user's question and avoid the failure modes users have flagged";
    namedSections.push({
      key: "feedback_signal",
      text: `Human feedback signal: Users rated recent responses ${upRatePct}% positive (n=${feedbackTotal}). ${guidance}.`,
    });
  }

  // Frequent user complaints — tag-based.
  if (topComplaintTags.length > 0) {
    namedSections.push({
      key: "feedback_complaints",
      text: `Frequent user complaints: ${topComplaintTags.join(", ")} — guard against these in your next response.`,
    });
  }
  ...
  if (flaggedList.length > 0) {
    namedSections.push({
      key: "known_bad",
      text: `Known-bad tools in this workspace: ${flaggedList.join(", ")} — avoid unless no alternative.`,
    });
  }
  ...
  if (preferredList.length > 0) {
    namedSections.push({
      key: "preferred",
      text: `Reliable tools in this workspace: ${preferredList.join(", ")} — prefer when multiple tools could satisfy the task.`,
    });
  }
  ...
  if (topFailTools.length > 0) {
    ...
    if (patternLines.length > 0) {
      namedSections.push({
        key: "patterns",
        text: `Common failure patterns: ${patternLines.join("; ")}.`,
      });
    }
  }
  ...
  if (conflicts.length >= MIN_CONFLICTS) {
    ...
    if (med != null && med < CONFLICT_SIMILARITY_MEDIAN_CEILING) {
      ...
      if (topic.length > 0) {
        namedSections.push({
          key: "conflicts",
          text: `Specialist disagreements have clustered around: ${topic.join(", ")}.`,
        });
      } else {
        namedSections.push({
          key: "conflicts",
          text: `Specialist disagreements have clustered across ${conflicts.length} recent events; double-check reconciliation.`,
        });
      }
    }
  }
  ...
  // Canonical order of sections in the output (must be stable across calls).
  ...
  // Drop priority (least valuable first) when over budget. Feedback signals
  // are kept because they reflect the freshest ground truth.
  const dropPriority = ["conflicts", "patterns", "preferred", "known_bad"];

  function assembleContent(keptKeys) {
    const keptSet = new Set(keptKeys);
    const ordered = canonicalOrder
      .map((k) => namedSections.find((s) => s.key === k && keptSet.has(k)))
      .filter(Boolean);
    return ordered.map((s) => s.text).join("\n");
  }

  let keptKeys = namedSections.map((s) => s.key);
  let content = assembleContent(keptKeys);
  for (const dropKey of dropPriority) {
    if (content.length <= MAX_PATCH_CHARS) break;
    if (keptKeys.includes(dropKey)) {
      keptKeys = keptKeys.filter((k) => k !== dropKey);
      content = assembleContent(keptKeys);
    }
  }
