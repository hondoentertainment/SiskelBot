...
export async function detectContradictions(conversationId, workspaceId = "default") {
  ...
  // Strip negation words only (keep the surrounding verbs like "is"/"are"
  // so "X is passing" and "X is not passing" yield the same skeleton).
  const stripNegation = (s) =>
    s
      .replace(/\b(isn't|aren't|wasn't|weren't|doesn't|don't|can't|won't|cannot|never|not|no)\b/g, "")
      .replace(/\s+/g, " ")
      .trim();
