    ...
    toolCalls
      .map((tc) => {
        ...
        const args = tc.args && typeof tc.args === "object" && !Array.isArray(tc.args)
          ? Object.fromEntries(Object.keys(tc.args).sort().map((k) => [k, tc.args[k]]))
          : {};
...
/** Phase 73.3: Named stagnation patterns + intervention recipes. */
export const STAGNATION_PATTERNS = [
  {
    id: "exact_cycle",
    match: (r) => String(r?.reason || "").startsWith("exact_cycle"),
    intervention: "Inject a reassess prompt and forbid repeating the last tool args fingerprint.",
  },
  {
    id: "partial_repetition",
    match: (r) => String(r?.reason || "").startsWith("partial_repetition"),
    intervention: "Force tool_choice=none for one turn and ask for a plan update.",
  },
  {
    id: "low_diversity",
    match: (r) => String(r?.reason || "").startsWith("low_diversity"),
    intervention: "Suggest alternate tools from the allowlist and shrink the working set.",
  },
];

/**
 * @param {{ stagnant?: boolean, reason?: string }} detection
 */
export function resolveStagnationIntervention(detection) {
  if (!detection?.stagnant && detection !== true) {
    return { patternId: null, intervention: null };
  }
  const reason = typeof detection === "object" ? detection.reason : "";
  const hit = STAGNATION_PATTERNS.find((p) => p.match({ reason }));
  return {
    patternId: hit?.id || "generic",
    intervention: hit?.intervention || "Stop the loop and answer from current context.",
  };
}
