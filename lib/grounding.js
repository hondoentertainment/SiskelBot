...
/** When set with AGENT_REQUIRE_CITATIONS, require at least one literal `[documentId]` for ids from search_context snippets. */
export function citationBracketIdsRequired() {
  return process.env.AGENT_CITATION_BRACKET_IDS === "1";
}
...
/**
 * Structured-style citation: answer must contain `[id]` for at least one known document id.
 * @param {string} answer
 * @param {Set<string>} ids
 */
export function answerReferencesCitationBracketIds(answer, ids) {
  const text = typeof answer === "string" ? answer : "";
  if (!text.trim()) return false;
  for (const id of ids) {
    if (id && text.includes(`[${id}]`)) return true;
...
export function checkCitationCoverage(finalAnswer, messages) {
  ...
  let ok;
  if (citationBracketIdsRequired() && ids.size > 0) {
    ok = answerReferencesCitationBracketIds(finalAnswer, ids);
    if (!ok) {
      return {
        ok: false,
        skipped: false,
        reason: "final_answer_missing_bracket_citation_for_retrieved_source_ids",
      };
    }
  } else {
    ok = answerReferencesCitation(finalAnswer, ids, titles);
    if (!ok) {
      return { ok: false, skipped: false, reason: "final_answer_missing_citation_for_retrieved_sources" };
    }
  }
