...
/** @type {Set<string>} */
import { validateSemantic, semanticValidationEnabled } from "./tool-semantic-validators.js";
...
function validateNamedToolCall(name, args, opts = {}) {
  ...
  // Semantic layer: only run when per-tool type/shape checks passed so we
  // don't double-report on obvious shape failures.
  if (errors.length === 0 && semanticValidationEnabled()) {
    const sem = validateSemantic(name, args);
    if (!sem.ok) {
      ...
      return {
        valid: false,
        errors,
        repairHint: sem.repairHint || errors.join("; "),
      };
    }
  }
...
/**
 * Async wrapper around the legacy validateToolCall(name, args, opts) path that
 * adds an inline auto-repair step when semantic validation fails.
 *
 * Behavior:
 *   - Always runs the synchronous validator first (schema+switch+semantic).
 *   - If valid, returns the sync result verbatim.
 *   - If invalid AND opts.repairFn is a function, calls repairFn once with
 *     { toolName, args, repairHint } and awaits its promise. If repair returns
 *     { ok: true, repairedArgs }, re-runs the sync validator on the repaired
 *     args. If that passes, returns { valid: true, errors: [], repairHint: "",
 *     repairedArgs } so the caller can swap the original args for the repaired
 *     ones before executing the tool.
 *   - Otherwise, returns the original failure unchanged.
 *
 * Backwards-compatible: callers without repairFn get the exact same result
 * object as validateToolCall would return (plus an extra awaited tick).
 *
 * @param {string|null|undefined} name
 * @param {object} args
 * @param {{ parseError?: string|null, repairFn?: (params: { toolName: string, args: object, repairHint: string }) => Promise<{ ok: boolean, repairedArgs?: object, error?: string }> }} [opts]
 * @returns {Promise<SiskelBot.ToolValidationResult & { repairedArgs?: object }>}
 */
export async function validateToolCallWithRepair(name, args, opts = {}) {
  const initial = validateNamedToolCall(name, args, { parseError: opts.parseError });
  if (initial.valid) return initial;

  const repairFn = opts.repairFn;
  if (typeof repairFn !== "function") return initial;

  // A parse error means the args object itself was never populated; there is
  // nothing meaningful to hand to a repair model, so skip.
  if (opts.parseError) return initial;

  let repairResult;
  try {
    repairResult = await repairFn({
      toolName: name,
      args,
      repairHint: initial.repairHint || initial.errors.join("; "),
    });
  } catch {
    return initial;
  }

  if (!repairResult || repairResult.ok !== true || !repairResult.repairedArgs) {
    return initial;
  }

  const repairedArgs = repairResult.repairedArgs;
  if (!repairedArgs || typeof repairedArgs !== "object" || Array.isArray(repairedArgs)) {
    return initial;
  }

  const recheck = validateNamedToolCall(name, repairedArgs, {});
  if (!recheck.valid) return initial;
