 ...
 * Precedence: `parallelAgents: false` → intent only; `true` → fan-out; unset → `SWARM_PARALLEL_AGENTS=1`.
 *
 * @param {{ researcher: boolean, executor: boolean }} intent
 * @param {object} [agentOptions]
 * @returns {{ specialistNames: string[], parallelFanOut: boolean }}
 */
export function resolveSwarmSpecialistNames(intent, agentOptions = {}) {
  ...
  if (agentOptions && agentOptions.parallelAgents === true) parallelFanOut = true;
  if (agentOptions && agentOptions.parallelAgents === false) parallelFanOut = false;

  if (parallelFanOut) {
    return { specialistNames: ["researcher", "executor"], parallelFanOut: true };
  }
  const eligible = [];
  ...
  return { specialistNames, parallelFanOut: false };
}
...
export async function runSwarm(req, res, config, model, options = {}) {
  ...
  const { specialistNames, parallelFanOut } = resolveSwarmSpecialistNames(intent, agentOpts);
