...
import { analyzeSwarmConflicts } from "./swarm-conflict-resolver.js";
...
export async function runSwarmDirect(specialistNames, query, opts = {}) {
  ...
  try {
    ...
    if (successful.length >= 2) {
      const analysis = analyzeSwarmConflicts(
        successful.map((r) => ({ specialist: r.specialist, output: r.output, success: r.success })),
      );
      if (analysis) {
        conflicts = {
