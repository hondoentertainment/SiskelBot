...
import { augmentMessagesWithWorkspaceAgent } from "./workspace-agent-settings.js";
  ...
  const results = await Promise.all(
    specialistNames.map(async (name) => {
      ...
      try {
        ...
        if (name === "researcher") {
          const parts = {};
          if (tools.includes("list_context")) {
            parts.list_context = JSON.parse((await runTool("list_context", {}, toolCtx)).content);
          }
          if (tools.includes("search_context")) {
            parts.search_context = JSON.parse((await runTool("search_context", { query: q || "general" }, toolCtx)).content);
          }
          if (tools.includes("semantic_search_context")) {
            parts.semantic_search_context = JSON.parse(
              (await runTool("semantic_search_context", { query: q || "general" }, toolCtx)).content
            );
          }
          if (tools.includes("get_context_document") && CONTEXT_DOC_ID_RE.test(q)) {
            parts.get_context_document = JSON.parse(
              (await runTool("get_context_document", { id: q }, toolCtx)).content
            );
          }
          ...
          const parts = {};
          if (tools.includes("list_recipes")) {
            parts.list_recipes = JSON.parse((await runTool("list_recipes", {}, toolCtx)).content);
          }
          if (tools.includes("get_recipe")) {
            parts.get_recipe = JSON.parse((await runTool("get_recipe", { name: q || "default" }, toolCtx)).content);
          }
          output = JSON.stringify(parts);
          success = Object.keys(parts).length > 0;
 ...
 * @param {object} opts - { url, config, model, toolCtx, backendFetch, maxIterations, storageUserId, workspace }
...
async function runSpecialistLoop(specialistName, messages, opts) {
  ...
  const { url, config, model, toolCtx, backendFetch, maxIterations = 5, storageUserId, workspace } = opts;
