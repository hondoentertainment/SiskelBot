...
import { runToolWithTransientRetries, resolveEffectiveMaxTransientRetries } from "./agent-tool-retry.js";
import { agentSessionApiEnabled, assertAgentSessionForRun } from "./agent-session.js";
 ...
 */
async function buildDirectSwarmToolCtx(opts = {}) {
  const workspace = opts.workspace || WORKSPACE;
  const workspaceUserId =
    typeof opts.workspaceUserId === "string" && opts.workspaceUserId.trim()
      ? opts.workspaceUserId.trim()
      : "anonymous";
  const wsSettings = await loadWorkspaceAgentSettings(workspaceUserId, workspace);
  const workspaceAllowedTools =
    wsSettings.allowedTools?.length > 0 ? new Set(wsSettings.allowedTools) : null;
  const denied = new Set(
    (wsSettings.agentPolicy?.deniedTools || []).map((x) => String(x).trim()).filter(Boolean)
  );
  const wr = (process.env.WORKSPACE_ROOT || "").trim();
  return {
    ...
    workspaceAllowedToolNames: wsSettings.allowedTools?.length ? wsSettings.allowedTools : undefined,
    workspaceDeniedTools: denied,
    ...(wr ? { workspaceFilesystemRoot: wr } : {}),
  };
}
...
export function resolveSwarmSpecialistNames(intent, agentOptions = {}) {
  ...
  if (parallelFanOut) {
    ...
    if (parallelNames.length === 0) {
      /* cannot fan out; fall through to intent-based roster */
 ...
 * @param {string} query
 * @param {{ workspace?: string, workspaceUserId?: string, allowExecution?: boolean, projectDir?: string, vercelToken?: string }} [opts]
 * @returns {Promise<SwarmDirectResult>}
 */
export async function runSwarmDirect(specialistNames, query, opts = {}) {
  const runId = randomUUID();
  const workspace = opts.workspace || WORKSPACE;
  const toolCtx = await buildDirectSwarmToolCtx(opts);
...
async function runSpecialistLoop(specialistName, messages, opts) {
  ...
  const wsAgentSpec = await loadWorkspaceAgentSettings(su, ws);
  const effectivePolicy = resolveEffectivePolicy(wsAgentSpec.agentPolicy || {});
  const deniedSpec = new Set(
    (wsAgentSpec.agentPolicy?.deniedTools || []).map((x) => String(x).trim()).filter(Boolean)
  );
  const policyState = createPolicyState(effectivePolicy, { deniedTools: deniedSpec });
  ...
  for (let i = 0; i < maxIterations; i++) {
    ...
    // Emit cost.update on the parent swarm session per specialist completion.
    if (opts.liveSessionId && opts.swarmRunId && data?.usage) {
      emitCostUpdate({
        sessionId: opts.liveSessionId,
        runId: opts.swarmRunId,
        model: specialistModel,
        usage: data.usage,
      });
    }
    ...
    const maxToolExtraRetries = resolveEffectiveMaxTransientRetries(wsAgentSpec.agentPolicy);
    const results = await Promise.all(
      toolCalls.map(async (tc) => {
        ...
        if (!policyCheck.ok) {
          toolCallsLog.push({ name, args, iteration, policyError: true, policyCode: policyCheck.code });
          return {
            tc,
            result: {
              content: JSON.stringify({
        ...
        let result;
        try {
          const wrapped = await runToolWithTransientRetries({
            toolName: name,
            policyState,
          ...
          if (wrapped.policyBlockedOnRetry) {
            result = { content: wrapped.content, ok: false };
          } else {
            result = {
              content: wrapped.content,
              ok: wrapped.ok,
              ...(wrapped._timeout ? { _timeout: true, _timeoutMs: wrapped._timeoutMs } : {}),
            };
          }
          if (result._timeout) {
    ...
    if (!afterRound.ok) {
      lastContent = "(Specialist stopped: policy budget exceeded)";
      break;
    }
  }
...
export async function runSwarm(req, res, config, model, options = {}) {
  ...
  const swarmSessionId = typeof agentOpts.sessionId === "string" ? agentOpts.sessionId.trim() : "";
  if (swarmSessionId) {
    if (!agentSessionApiEnabled()) {
      throw new Error("Agent sessions API disabled (AGENT_SESSION_API=0); remove sessionId from agentOptions.");
    }
    const sessOk = await assertAgentSessionForRun(swarmSessionId, workspace, storageUserId);
    if (!sessOk.ok) {
      throw new Error(sessOk.message || "Invalid agent session");
    }
  }
  ...
  const swarmDenied = new Set(
    (wsAgentSettings.agentPolicy?.deniedTools || []).map((x) => String(x).trim()).filter(Boolean)
  );
  const swarmRoot = (process.env.WORKSPACE_ROOT || "").trim();
  ...
  const toolCtx = {
    ...
    workspaceDeniedTools: swarmDenied,
    ...(swarmRoot ? { workspaceFilesystemRoot: swarmRoot } : {}),
    ...(swarmSessionId ? { agentSessionId: swarmSessionId } : {}),
  };
    ...
    await withSpan("swarm.synthesizer", { "specialist.name": "synthesizer", "specialist.model": synthModel }, async (synthSpan) => {
      try {
        ...
        if (!synthResponse.ok) {
          ...
          if (swarmSessionId && data?.usage) {
            emitCostUpdate({ sessionId: swarmSessionId, runId, model: synthModel, usage: data.usage });
          }
