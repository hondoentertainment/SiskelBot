 ...
 * @typedef {import("./types.d.ts").SwarmIntent} SwarmIntent
 * @typedef {import("./types.d.ts").SwarmSpecialistResolution} SwarmSpecialistResolution
 * @typedef {import("./types.d.ts").SwarmDirectResult} SwarmDirectResult
 * @typedef {import("./types.d.ts").SwarmResult} SwarmResult
 * @typedef {import("./types.d.ts").SwarmRunOptions} SwarmRunOptions
 * @typedef {import("./types.d.ts").SpecialistLoopOptions} SpecialistLoopOptions
 * @typedef {import("./types.d.ts").SpecialistResult} SpecialistResult
 */
import { randomUUID } from "crypto";
import { emitEvent } from "./webhooks.js";
import { getSpecialist, getSpecialists, getModelForSpecialist } from "./specialists.js";
import { recordSwarm, recordAgentPhaseMs, recordAgentRunSummary, recordSwarmConflict } from "./metrics.js";
...
import { analyzeSwarmConflicts, resolveConflictWithJudge } from "./swarm-conflict-resolver.js";
import { recordConflict as recordSwarmConflictEvent, swarmConflictLogEnabled } from "./swarm-conflict-store.js";
...
import { debateEnabled, runDebateRound } from "./swarm-debate.js";
import { recordSpecialistObservation, rankSpecialists, learnedRoutingEnabled } from "./specialist-metrics.js";

const swarmTracer = trace.getTracer("siskel-bot-swarm", "1.0.0");
...
export function resolveSwarmSpecialistNames(intent, agentOptions = {}) {
  ...
  if (parallelFanOut) {
    ...
    if (parallelNames.length === 0) {
      // fall through to intent-based selection below
 ...
 * @param {string} query
 * @param {{ workspace?: string, allowExecution?: boolean, projectDir?: string, vercelToken?: string }} [opts]
 * @returns {Promise<SwarmDirectResult>}
 */
export async function runSwarmDirect(specialistNames, query, opts = {}) {
  ...
  try {
    const successful = results.filter((r) => r.success && typeof r.output === "string");
    if (successful.length >= 2) {
      const normalized = successful.map((r) => ({ specialist: r.specialist, output: r.output, success: r.success }));
      normalizedForLog = normalized;
      // When the caller supplies a judge, run end-to-end resolution; otherwise
      // just surface the conflict metadata without calling a model.
      if (typeof opts.judgeFn === "function") {
        const resolved = await resolveConflictWithJudge({
          responses: normalized,
          judgeFn: opts.judgeFn,
          opts: { timeoutMs: opts.judgeTimeoutMs },
        });
        if (resolved) {
          conflicts = {
            type: resolved.report.conflictType,
            similarity: resolved.report.similarity,
            summary: resolved.report.summary,
            specialists: resolved.report.specialists,
            reconciliationPrompt: resolved.prompt,
            ...(resolved.reconciled ? { reconciled: resolved.reconciled } : {}),
            ...(resolved.error ? { judgeError: resolved.error } : {}),
          };
          try { recordSwarmConflict(resolved.report.conflictType); } catch { /* metrics best-effort */ }
        }
      } else {
        const analysis = analyzeSwarmConflicts(normalized);
        if (analysis) {
          conflicts = {
            type: analysis.report.conflictType,
            similarity: analysis.report.similarity,
            summary: analysis.report.summary,
            specialists: analysis.report.specialists,
            reconciliationPrompt: analysis.prompt,
          };
          try { recordSwarmConflict(analysis.report.conflictType); } catch { /* metrics best-effort */ }
        }
      }
    }
  } catch { /* conflict detection is best-effort */ }

  if (conflicts && swarmConflictLogEnabled()) {
    try {
      await recordSwarmConflictEvent(workspace, {
        type: conflicts.type,
        similarity: conflicts.similarity,
        summary: conflicts.summary,
        specialists: conflicts.specialists,
        responses: normalizedForLog || [],
        ...(conflicts.reconciled ? { reconciled: conflicts.reconciled } : {}),
        ...(conflicts.judgeError ? { judgeError: conflicts.judgeError } : {}),
      });
    } catch { /* conflict log is best-effort */ }
  }
 ...
 * @param {import("./types.d.ts").ChatMessage[]} messages - Chat messages (will be prefixed with system prompt)
 * @param {SpecialistLoopOptions} opts
...
async function runSpecialistLoop(specialistName, messages, opts) {
  ...
  for (let i = 0; i < maxIterations; i++) {
    ...
    const llmSpan = swarmTracer.startSpan("llm.chat_completion", {
      attributes: {
        "llm.model": specialistModel,
        "llm.iteration": i + 1,
    ...
    // Emit cost.update on the parent swarm run's session emitter so the
    // Agent Run hero footer reflects specialist-LLM spend alongside the
    // single-agent loop's own emissions.
    if (liveSessionId && parentRunId) {
      try {
        emitCostUpdate({
    ...
    const results = await Promise.all(
      toolCalls.map(async (tc) => {
        ...
        const toolExecT0 = Date.now();
        const toolSpan = swarmTracer.startSpan(`tool.${name}`, {
          attributes: {
        ...
        try {
          ...
          if (result._timeout) {
            ...
            toolSpan.setStatus({ code: SpanStatusCode.ERROR, message: "timeout" });
          ...
          toolSpan.setAttribute("agent.tool.duration_ms", Date.now() - toolExecT0);
          toolSpan.end();
        }
        return { tc, result };
      })
 ...
 * @param {import("express").Response} res - Express response (headers set during run)
 * @param {ProxyConfig} config - Proxy config from buildProxyConfig
 * @param {string} model - Default model name
 * @param {SwarmRunOptions} [options]
...
export async function runSwarm(req, res, config, model, options = {}) {
  ...
  let { specialistNames, parallelFanOut, rosterSource } = resolveSwarmSpecialistNames(intent, agentOpts);

  // Wave-2: Learned routing. When SWARM_LEARNED_ROUTING=1 and we have enough
  // observations, reorder specialists by historical score. No-op (falls back
  // to original order) when insufficient data or the flag is off.
  if (learnedRoutingEnabled() && specialistNames.length >= 2) {
    try {
      const ranked = await rankSpecialists(workspace, intent?.mode || "keyword", specialistNames);
      if (Array.isArray(ranked) && ranked.length === specialistNames.length) {
        specialistNames = ranked.map((r) => r.specialist);
        rosterSource = `${rosterSource}+learned`;
      }
    } catch { /* best-effort */ }
  }
  ...
  // Resolve liveSessionId so cost.update events can publish to the Agent Run
  // SSE hero. Prefer an explicit agentOptions.sessionId; otherwise try the
  // runId → sessionId reverse index (sync mirror first, persisted fallback).
  let liveSessionId =
  ...
  // Phase 32.5: record specialist delegation decision
  try {
    const allSpecialists = getSpecialists()
      .filter((s) => s.name !== "synthesizer")
      .map((s) => s.name);
    for (const name of specialistNames) {
      globalDecisionLog.recordDecision({
        ...
        chosen: name,
        alternatives: allSpecialists.filter((n) => n !== name),
        reasoning: `Roster source: ${rosterSource}. Intent detected researcher=${!!intent.researcher} executor=${!!intent.executor} via ${intent.mode || "keyword"}.`,
        confidence: rosterSource === "client" ? 0.95 : rosterSource === "parallel" ? 0.8 : 0.7,
        factors: [
          { name: "roster_source", weight: 0.5, value: rosterSource },
          { name: "intent_mode", weight: 0.3, value: intent.mode || "keyword" },
          { name: "parallel_fan_out", weight: 0.2, value: parallelFanOut },
        ],
        workspaceId: workspace,
      });
    }
  ...
  // Phase 69: parent span for entire swarm
  const swarmResult = await withSpan("swarm.run", { "swarm.run_id": runId, "swarm.specialists_count": specialistNames.length }, async (swarmSpan) => {

  const specialistResults = await Promise.all(
    specialistNames.map(async (name) => {
      const specialistModel = getModelForSpecialist(name, model);
      const t0 = Date.now();
      try {
        // Phase 69: span per specialist dispatch
        const { content } = await withSpan("swarm.specialist.dispatch", {
          "specialist.name": name,
          "specialist.model": specialistModel,
        }, async (specSpan) => {
          const r = await runSpecialistLoop(name, messages, {
          ...
          specSpan.setAttributes({ "specialist.duration_ms": dur });
          return r;
        });
        const latencyMs = Date.now() - t0;
        await emitEvent(
  ...
  // Record per-specialist quality observations (opt-in via always-on store;
  // routing decisions only consult them when SWARM_LEARNED_ROUTING=1).
  for (const r of specialistResults) {
    try {
      recordSpecialistObservation({
        workspace,
        specialist: r.specialist,
        intent: intent?.mode || "keyword",
  ...
  // Optional debate round before synthesis. Only runs when SWARM_DEBATE=1 and
  // there are at least 2 specialists with text output.
  if (debateEnabled() && specialistResults.length >= 2) {
    try {
      const revisions = await runDebateRound({
        results: specialistResults.map((r) => ({ specialist: r.specialist, content: r.content })),
        userMessage,
        backendFetch,
        url: `${config.baseUrl}${config.path}`,
        model,
      ...
      for (const rev of revisions) {
        if (rev.changed) {
          const target = specialistResults.find((r) => r.specialist === rev.specialist);
          if (target) target.content = rev.revised;
        }
      }
      // Wave-2: feed debate outcome back into specialist quality metrics.
      // Unchanged drafts get quality 0.8; revised drafts get 0.4 (peer found a flaw).
      for (const rev of revisions) {
        try {
          recordSpecialistObservation({
            workspace,
            specialist: rev.specialist,
            intent: intent?.mode || "keyword",
            latencyMs: 0,
            success: true,
            quality: rev.changed ? 0.4 : 0.8,
          });
        } catch { /* metric is best-effort */ }
      }
    } catch { /* debate is best-effort */ }
  }
  ...
  if (STREAM_SWARM_SYNTH) {
    ...
    // Phase 69: span for synthesizer step
    await withSpan("swarm.synthesizer", { "specialist.name": "synthesizer", "specialist.model": synthModel }, async (synthSpan) => {
      try {
        ...
        if (!synthResponse.ok) {
          ...
          synthSpan.setAttributes({ "specialist.success": false });
        } else {
          const data = await synthResponse.json().catch(() => ({}));
          const msg = data.choices?.[0]?.message;
          finalContent = typeof msg?.content === "string" ? msg.content : aggregateText;
          synthSpan.setAttributes({ "specialist.success": true });
          // Roll the synthesizer chat-completion's cost into the swarm run's
          // cumulative total so the footer stays monotonic after synth.
          if (liveSessionId) {
            try {
              emitCostUpdate({
                sessionId: liveSessionId,
      ...
      synthSpan.setAttributes({ "specialist.duration_ms": Date.now() - synthT0 });
    });
  ...
  // Phase 69: finalize parent swarm span
  swarmSpan.setAttributes({
    "swarm.duration_ms": durationMs,
    "swarm.success_count": successCount,
  });
  ...
  }); // end withSpan("swarm.run")

  // Bound the accumulator map to in-flight runs only.
