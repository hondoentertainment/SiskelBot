...
/** Repo-shipped fixtures; tests often set VERCEL=1 so STORAGE_ROOT is /tmp and has no copies of these files. */
const SHIPPED_DATA_DIR = join(MODULE_DIR, "..", "data");
const SHIPPED_EVAL_SETS_DIR = join(SHIPPED_DATA_DIR, "eval-sets");
const SHIPPED_EVAL_SINGLE = join(SHIPPED_DATA_DIR, "eval-sets.json");

/** Serialize legacy migration across duplicated module graphs (e.g. tests cache-busting server-configured-app). */
const migrateLegacySingleton = Symbol.for("siskel.storageEval.migrateLegacySingleton");
...
/**
 * Ensure eval-sets-store exists or legacy files have been migrated (process-global single-flight).
 * @returns {Promise<object>}
 */
async function ensureEvalStore() {
  const path = evalSetsPath();
  let data = await readJsonPath(path, null);
  if (data && data._version) return data;

  const g = globalThis;
  if (!g[migrateLegacySingleton]) {
    g[migrateLegacySingleton] = migrateLegacyIfNeeded();
  }
  await g[migrateLegacySingleton];
  data = await readJsonPath(path, null);
  if (data && data._version) return data;
 ...
 */
function mergeLegacySingleFile(absPath, sets) {
  ...
  try {
    ...
    for (const s of items) {
      if (s && s.id && !sets.some((r) => r.id === s.id)) sets.push(s);
    }
  } catch (_) {}
}

function mergeLegacySetsDir(absDir, sets) {
...
async function migrateLegacyIfNeeded() {
  ...
  mergeLegacySingleFile(singleFile, sets);
  mergeLegacySetsDir(setsDir, sets);

  // Fall back to fixtures next to this package when storage root is isolated (e.g. VERCEL tests → /tmp).
  if (singleFile !== SHIPPED_EVAL_SINGLE) {
    mergeLegacySingleFile(SHIPPED_EVAL_SINGLE, sets);
  }
  if (setsDir !== SHIPPED_EVAL_SETS_DIR) {
    mergeLegacySetsDir(SHIPPED_EVAL_SETS_DIR, sets);
  }
...
export async function listEvalSets() {
  const data = await ensureEvalStore();
