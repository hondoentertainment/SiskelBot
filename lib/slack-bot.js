...
/**
 * Update or reply to an interactive message via Slack's response_url.
 * Best-effort: returns { ok } and never throws on transport errors.
 */
export async function respondToSlackInteraction(responseUrl, message) {
  if (!responseUrl || typeof responseUrl !== "string") return { ok: false };
  ...
  try {
    ...
    console.error("[slack-bot] response_url post failed:", err.message);
    return { ok: false };
  }
}

/**
 * Handle a Slack interactive-component payload (button clicks etc.).
 * Currently dispatches initiative Approve/Dismiss actions. Returns
 * { status, body } for the HTTP layer; Slack requires a fast 200.
 */
export async function handleSlackInteraction(payload) {
  ...
  if (actionId !== "initiative_approve" && actionId !== "initiative_dismiss") {
    return { status: 200, body: { ok: true } };
  }

  let ref;
  try {
    ref = JSON.parse(action.value || "{}");
  } catch {
    return { status: 200, body: { ok: true } };
  }
  if (!ref.w || !ref.p) return { status: 200, body: { ok: true } };

  const by = payload.user?.username || payload.user?.name || payload.user?.id || "slack-user";
  const responseUrl = payload.response_url;

  // Resolve asynchronously so we can return the required fast 200 to Slack.
  setImmediate(async () => {
    try {
      const { resolveProposal } = await import("./initiative-engine.js");
      const status = actionId === "initiative_approve" ? "approved" : "dismissed";
      const updated = await resolveProposal(ref.w, ref.p, { status, by, resolution: "via Slack" });
      const verb = status === "approved" ? "Approved" : "Dismissed";
      const title = updated?.title || ref.p;
      await respondToSlackInteraction(responseUrl, {
        text: `:white_check_mark: ${verb} initiative "${title}" — by ${by}`,
        blocks: [
          {
            type: "section",
            text: {
              type: "mrkdwn",
              text: `:white_check_mark: *${verb}* initiative — ${title}\n_resolved by ${by}_`,
            },
          },
        ],
      });
    } catch (err) {
      console.error("[slack-bot] initiative interaction error:", err.message);
      await respondToSlackInteraction(responseUrl, {
        text: `Sorry, I couldn't update that initiative: ${err.message}`,
      });
    }
  });
