...
/**
 * Resolve the prompt patch text to inject for a specific run, honoring an
 * active canary deployment. Falls back to the workspace's currently-applied
 * patch when no canary is active or the run lands in the baseline arm.
 *
 * Returns "" when there's no patch to inject. Always safe to call.
 *
 * @param {string} workspace
 * @param {string} runId
 * @returns {Promise<{content: string, arm: "candidate"|"baseline"|"none", version: string|null}>}
 */
export async function resolvePatchContentForRun(workspace, runId) {
  const ws = typeof workspace === "string" && workspace.length > 0 ? workspace : "default";
  // Lazy-import the tuner functions we need to avoid a circular dep at module
  // load time (agent-prompt-canary.js already imports listPatches at the top).
  const { getActivePatchContent, listPatches } = await import("./agent-prompt-tuner.js");

  if (!canaryEnabled()) {
    const content = await getActivePatchContent(ws);
    return { content, arm: "none", version: null };
  }

  const active = await getActiveCanary(ws);
  if (!active || active.status !== "active" || Date.now() > active.endsAt) {
    const content = await getActivePatchContent(ws);
    return { content, arm: "none", version: null };
  }

  const useCandidate = await shouldUseCandidate(ws, runId);
  if (useCandidate) {
    // Find the candidate patch's content; if missing, gracefully fall back.
    const all = await listPatches(ws, { limit: 500 });
    const candidate = all.find((p) => p.version === active.candidateVersion);
    if (candidate && typeof candidate.content === "string" && candidate.content.length > 0) {
      return { content: candidate.content, arm: "candidate", version: candidate.version };
    }
  }

  const content = await getActivePatchContent(ws);
  return { content, arm: "baseline", version: active.baselineVersion };
}
