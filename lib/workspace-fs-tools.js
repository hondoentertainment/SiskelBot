 ...
 */
import { readdir, readFile, lstat, writeFile, mkdir, unlink, rmdir, rename } from "fs/promises";
...
export async function workspaceWriteFile(ctx, relativePath, content, opts = {}) {
  const root = getRoot(ctx);
  const resolved = resolveWorkspacePath(root, relativePath);
  if (!resolved.ok) return { ok: false, ...resolved };
  const maxBytes = Math.max(1, Number(opts.maxBytes) || DEFAULT_WRITE_MAX);
  const buf = Buffer.from(content, "utf8");
  if (buf.length > maxBytes) {
    return { ok: false, code: "FILE_TOO_LARGE", error: `Content ${buf.length} bytes exceeds limit ${maxBytes}` };
  }
  try {
    if (opts.createDirs !== false) {
      const { dirname: pathDirname } = await import("node:path");
      await mkdir(pathDirname(resolved.absPath), { recursive: true });
    }
    await writeFile(resolved.absPath, buf);
    return { ok: true, path: relativePath, byteLength: buf.length };
  } catch (e) {
    return { ok: false, code: "FS_ERROR", error: String(e?.message || e) };
  }
}

export async function workspacePatchFile(ctx, relativePath, unifiedDiff) {
  const root = getRoot(ctx);
  const resolved = resolveWorkspacePath(root, relativePath);
  if (!resolved.ok) return { ok: false, ...resolved };
  try {
    let original;
    try {
      original = await readFile(resolved.absPath, "utf8");
    } catch {
      original = "";
    }
    ...
    while (i < diffLines.length) {
      ...
      if (dl.startsWith("@@")) {
        ...
        if (!m) { i++; continue; }
        const srcStart = Math.max(0, parseInt(m[1], 10) - 1);
        while (srcIdx < srcStart) {
          output.push(lines[srcIdx]);
          srcIdx++;
        }
        i++;
        while (i < diffLines.length && !diffLines[i].startsWith("@@") && !diffLines[i].startsWith("diff ")) {
          const dl2 = diffLines[i];
          if (dl2.startsWith("-")) {
            srcIdx++;
            linesRemoved++;
          } else if (dl2.startsWith("+")) {
            output.push(dl2.slice(1));
            linesAdded++;
          } else {
            output.push(lines[srcIdx]);
            srcIdx++;
    ...
    await writeFile(resolved.absPath, patched, "utf8");
    return { ok: true, path: relativePath, linesAdded, linesRemoved };
  } catch (e) {
    return { ok: false, code: "FS_ERROR", error: String(e?.message || e) };
  }
}

export async function workspaceDeletePath(ctx, relativePath) {
  const root = getRoot(ctx);
  const rel = typeof relativePath === "string" ? relativePath.trim() : "";
  if (!rel || rel === "." || rel === "/") {
    return { ok: false, code: "FORBIDDEN", error: "Refusing to delete workspace root" };
  }
  const resolved = resolveWorkspacePath(root, rel);
  if (!resolved.ok) return { ok: false, ...resolved };
  try {
...
export async function workspaceCreateDir(ctx, relativePath) {
  const root = getRoot(ctx);
  const resolved = resolveWorkspacePath(root, relativePath);
  if (!resolved.ok) return { ok: false, ...resolved };
  try {
...
export async function workspaceMoveFile(ctx, fromPath, toPath) {
  const root = getRoot(ctx);
  ...
  try {
    const { dirname: pathDirname } = await import("node:path");
    await mkdir(pathDirname(to.absPath), { recursive: true });
