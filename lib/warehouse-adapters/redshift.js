...
async function loadRedshiftDataSdk() {
  try {
    ...
    throw new Error(`redshift adapter: @aws-sdk/client-redshift-data not available: ${e.message}`);
  }
}
...
async function loadS3Sdk() {
  try {
    ...
    throw new Error(`redshift adapter: @aws-sdk/client-s3 not available: ${e.message}`);
  }
}
