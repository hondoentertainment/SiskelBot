 ...
 * Optional Undici Agent for HTTP keep-alive to upstream (BACKEND_FETCH_KEEPALIVE=0 to disable).
 */
import { isOpen } from "./circuit-breaker.js";
import { fetch as undiciFetch, Agent } from "undici";

const USE_BACKEND_KEEPALIVE = process.env.BACKEND_FETCH_KEEPALIVE !== "0";
const backendDispatcher = USE_BACKEND_KEEPALIVE
  ? new Agent({
      keepAlive: true,
      keepAliveTimeout: Math.min(300_000, Number(process.env.BACKEND_KEEPALIVE_TIMEOUT_MS) || 60_000),
      connections: Math.min(512, Math.max(32, Number(process.env.BACKEND_MAX_CONNECTIONS) || 256)),
    })
  : null;

async function dispatchFetch(url, options) {
  if (backendDispatcher) {
    return undiciFetch(url, { ...options, dispatcher: backendDispatcher });
  }
...
export async function fetchWithTimeoutAndRetry(url, options, backend = "ollama") {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      const res = await dispatchFetch(url, mergedOptions);
