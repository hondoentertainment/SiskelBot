...
// Phase 51.4: unified policy audit trail.
...
export async function logClassificationEvent(event) {
  ...
  // Phase 51.4: mirror a summary of this classification into the audit trail.
  try {
    const r = record.report || {};
    const action = r.safe === false ? "block" : "allow";
    const severity = typeof r.severity === "number" ? r.severity : null;
    await _recordPolicyDecision({
      workspaceId: record.workspaceId,
      policyId: event.policyId || "output-classifier-default",
      source: "output-classifiers",
      action,
      severity,
      userId: record.userId,
      reason: r.safe === false
        ? `flags=${[r.toxicity?.toxic && "toxicity", r.pii?.found && "pii", r.bias?.biased && "bias", r.hallucination?.likelyHallucination && "hallucination"].filter(Boolean).join(",")}`
        : null,
      metadata: { classifierSource: record.source },
    });
  } catch (_) { /* never break on audit failure */ }
