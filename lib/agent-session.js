...
import { lintPlanDag } from "./agent-session-plan-dag.js";
...
export function validateAgentSessionPlanInput(body) {
  ...
  const dagLintMode = (process.env.AGENT_SESSION_PLAN_DAG_LINT || "").trim().toLowerCase();
  if (dagLintMode === "strict" && planDag) {
    const lint = lintPlanDag(planDag);
    if (!lint.ok && !lint.skipped) {
      return {
        ok: false,
        code: "PLAN_DAG_LINT_FAILED",
        message: lint.issues.join("; "),
      };
    }
  }
...
/**
 * @param {{ workspace: string, ownerStorageUserId: string, title?: string, planSummary?: string, planDag?: object, parentSessionId?: string | null, depth?: number }} opts
 */
export async function createAgentSession(opts) {
  ...
  await withPathLock(path, async () => {
    ...
    if (parentSessionId) {
      const parent = data.sessions[parentSessionId];
      if (!parent) {
        const err = new Error("Parent session not found");
        err.code = "PARENT_NOT_FOUND";
        throw err;
      }
      const parentDepth = typeof parent.depth === "number" ? parent.depth : 0;
      session.parentSessionId = parentSessionId;
      session.depth = parentDepth + 1;
      const children = Array.isArray(parent.childSessionIds) ? parent.childSessionIds : [];
...
/**
 * Return direct children of a session (shallow, not recursive).
 *
 * @param {string} sessionId
 * @returns {Promise<Array<{ id: string, title: string, status: string, depth: number, costUsd?: number, createdAt: string }>>}
 */
export async function getSessionChildren(sessionId) {
  ...
  if (!session) return [];
  const childIds = Array.isArray(session.childSessionIds) ? session.childSessionIds : [];
  const results = [];
  for (const cid of childIds) {
    const child = data.sessions[cid];
    if (!child) continue;
    results.push({
      id: child.id,
      title: child.title || "",
      ...
      ...(child.costUsd != null ? { costUsd: child.costUsd } : {}),
...
/**
 * Build a recursive session tree using breadth-first traversal.
 * Caps at MAX_SESSION_DEPTH (5) to prevent unbounded recursion.
 *
 * @param {string} sessionId
 * @returns {Promise<{ id: string, title: string, status: string, depth: number, costUsd?: number, children: object[] } | null>}
 */
export async function getSessionTree(sessionId) {
  ...
  const path = storePath();
  const data = normalizeStore(await readJsonPath(path, null));
  const root = data.sessions[id];
  if (!root) return null;

  function buildNode(sid) {
  ...
  // BFS traversal with depth cap
  const queue = [rootNode];
  while (queue.length > 0) {
    const node = queue.shift();
    if (node.depth >= MAX_SESSION_DEPTH) continue;
    const sess = data.sessions[node.id];
    if (!sess) continue;
    const childIds = Array.isArray(sess.childSessionIds) ? sess.childSessionIds : [];
    for (const cid of childIds) {
...
/**
 * Return the path from root to the given session: [rootId, ..., parentId, sessionId].
 *
 * @param {string} sessionId
 * @returns {Promise<string[]>}
 */
export async function getSessionAncestors(sessionId) {
  const id = String(sessionId || "").trim();
  if (!id) return [];
  const path = storePath();
  const data = normalizeStore(await readJsonPath(path, null));
  ...
  while (current) {
    if (visited.has(current)) break; // guard against cycles
    visited.add(current);
    const sess = data.sessions[current];
    if (!sess) break;
    ancestors.unshift(current);
    current = typeof sess.parentSessionId === "string" && sess.parentSessionId.trim()
      ? sess.parentSessionId.trim()
      : null;
  }
  return ancestors;
}
