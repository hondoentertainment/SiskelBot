...
export async function remember(userId, workspaceId, memory) {
  ...
  // Wave-2: Async consolidation. Off the hot path — fired via setImmediate so
  // remember() returns immediately. The AGENT_MEMORY_CONSOLIDATE_SYNC=1 legacy
  // flag preserves the blocking behavior for callers that depend on in-line
  // consolidation (e.g. tests asserting pre-write state).
  const activeCount = memories.filter((m) => !m.deleted).length;
  const nearCap = activeCount >= Math.floor(MAX_MEMORIES * 0.8);
  if (nearCap && process.env.AGENT_MEMORY_CONSOLIDATE_SYNC === "1") {
    try {
    ...
    setImmediate(() => {
      import("./agent-memory-consolidate.js")
        .then((mod) => mod.consolidateIfNeeded(userId, workspaceId))
        .catch(() => { /* best-effort */ });
    });
  }
 ...
 * @param {string} workspaceId
 * @param {string} query
 * @param {object} options - { category?, limit?, minImportance?, dateFrom?, dateTo? }
 * @returns {Promise<{ memories: Array<{ id, content, category, importance, createdAt, relevance }> }>}
 */
export async function recall(userId, workspaceId, query, options = {}) {
  ...
  const scored = filtered
    .map((m) => {
      ...
      const relevance = Math.round((matchCount / words.length) * 100) / 100;
      return {
        id: m.id,
        content: m.content,
        category: m.category,
  ...
  // Sort by relevance desc, then importance desc
  scored.sort((a, b) => b.relevance - a.relevance || b.importance - a.importance);
