...
function getOAuthUsersPath() {
  const dataRoot = process.env.STORAGE_PATH || join(process.cwd(), "data");
  return join(dataRoot, "oauth-users.json");
}

function ensureDataDir() {
  const dir = dirname(getOAuthUsersPath());
  if (!existsSync(dir)) mkdirSync(dir, { recursive: true });
}

function loadOAuthUsers() {
  const OAUTH_USERS_PATH = getOAuthUsersPath();
  try {
...
function saveOAuthUsers(users) {
  ensureDataDir();
  writeFileSync(getOAuthUsersPath(), JSON.stringify({ _version: 1, users }, null, 0), "utf8");
}
