 ...
 *
 * Plan-tier quotas are read from `billing:<workspaceId>` in storage (no import
 * from lib/billing.js to avoid cycles). If no subscription is found the workspace
 * is treated as the free plan.
 */

const TENANT_QUOTAS_TYPE = "tenant_quotas";

const PLAN_LIMITS = {
  free:       { requestsPerMinute: 10,  tokensPerDay: 50_000,     storageBytes: 100 * 1024 * 1024 },
  pro:        { requestsPerMinute: 60,  tokensPerDay: 1_000_000,  storageBytes: 5 * 1024 * 1024 * 1024 },
  enterprise: { requestsPerMinute: 300, tokensPerDay: 10_000_000, storageBytes: 50 * 1024 * 1024 * 1024 },
};
...
/**
 * Resolve the subscription plan for a workspace directly from storage.
 * Returns the plan name string ("free" | "pro" | "enterprise"), or null when
 * no billing record exists (caller should fall back to env defaults in that case).
 */
async function resolvePlan(workspaceId, storage) {
  if (!storage || typeof storage.get !== "function") return null;
  try {
    const billingData = await storage.get(`billing:${workspaceId}`, "default");
...
/**
 * Look up per-workspace quota override (if any), merged with plan-tier limits.
 * Priority: admin override fields > plan-tier defaults (if subscription found) > env defaults.
 * Falls back gracefully when storage is missing or throws.
 */
async function getQuotasForWorkspace(workspaceId, storage) {
  const envDefaults = readDefaults();
  const plan = await resolvePlan(workspaceId, storage);
  const planLimits = plan ? PLAN_LIMITS[plan] : null;

  const base = planLimits
    ? {
        requestsPerMinute: planLimits.requestsPerMinute,
        tokensPerDay: planLimits.tokensPerDay,
        storageBytesMax: planLimits.storageBytes,
        plan,
        source: "plan",
      }
    : { ...envDefaults, plan: "free", source: "plan" };

  if (!storage || typeof storage.get !== "function") return base;
  ...
  try {
    ...
    // Storage failure -> use base (fail open)
  }
  return base;
}
...
/**
 * Read effective quotas for a workspace.
 * Returns merged plan defaults + any admin overrides, plus metadata:
 * { requestsPerMinute, tokensPerDay, storageBytes, storageBytesMax, plan, source }
 */
export async function getEffectiveQuotas(workspaceId, storage) {
  const ws = String(workspaceId || "default");
  const q = await getQuotasForWorkspace(ws, storage);
  return {
    requestsPerMinute: q.requestsPerMinute,
    tokensPerDay: q.tokensPerDay,
    storageBytes: q.storageBytesMax,
    storageBytesMax: q.storageBytesMax,
    plan: q.plan ?? "free",
...
/**
 * Check whether a workspace can store `bytesToAdd` more bytes.
 * Reads current usage from the in-memory counter set by recordStorageBytes().
 * Returns { allowed, current, limit, plan }.
 */
export async function checkStorageQuota(workspaceId, bytesToAdd, storage) {
  const ws = String(workspaceId || "default");
  const q = await getQuotasForWorkspace(ws, storage);
  const current = storageBytes.get(ws) || 0;
  const limit = q.storageBytesMax;
  const allowed = current + (Number(bytesToAdd) || 0) <= limit;
  return { allowed, current, limit, plan: q.plan ?? "free" };
}
