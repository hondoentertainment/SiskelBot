...
export function isPrReviewHitlEnabled() {
  return process.env.PR_REVIEW_HITL === "1";
}

function resolveReviewStatus({ requireApproval } = {}) {
  if (isPrReviewHitlEnabled() || requireApproval === true) return "draft";
  return "approved";
}
 ...
 */
export async function reviewDiff({ workspaceId, diff, rules, requireApproval } = {}) {
  ...
  const review = {
    ...
    comments: filtered,
    status: resolveReviewStatus({ requireApproval }),
  };
...
export async function approveReview(reviewId, workspaceId, { approvedBy } = {}) {
  const ws = sanitizeId(workspaceId);
  const file = storePath(ws);
  return withPathLock(file, async () => {
    ...
    if (review.status === "approved") {
      const err = new Error("review already approved");
      err.code = "ALREADY_APPROVED";
      throw err;
    }
    review.status = "approved";
    review.approvedAt = Date.now();
    review.approvedBy = approvedBy ?? null;
    reviews[idx] = review;
    await writeJsonPath(file, { reviews });
    return review;
  });
}
...
export async function reviewDiffWithLLM({
  ...
  if (!llmClient) {
    return reviewDiff({ workspaceId, diff, requireApproval });
  }
  const ws = sanitizeId(workspaceId);
  if (typeof diff !== "string" || !diff.trim()) {
    throw new Error("diff is required");
