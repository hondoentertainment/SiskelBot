...
export async function executeAgentToolBatch(opts) {
  ...
  // Optional post-execution result judging (AGENT_TOOL_RESULT_JUDGE=1):
  // an LLM verdict on whether each result serves the user's intent; on a
  // negative verdict an advisory is appended so the next iteration can adapt.
  // Fail-open: judging errors never affect the tool results.
  try {
    const { toolResultJudgeEnabled, judgeToolResult } = await import("./tool-result-judge.js");
    const userIntent = typeof opts.userIntent === "string" ? opts.userIntent : "";
    if (toolResultJudgeEnabled() && userIntent.trim()) {
      const judgeModel = typeof toolCtx.currentModel === "string" ? toolCtx.currentModel : "default";
      const callFn = async (prompt) => {
        const cfg = toolCtx.buildProxyConfig(judgeModel);
        const resp = await toolCtx.backendFetch(`${cfg.baseUrl}${cfg.path}`, {
          method: "POST",
          headers: cfg.headers,
          body: JSON.stringify({
            model: judgeModel,
        ...
        if (!resp.ok) throw new Error(`judge backend ${resp.status}`);
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        if (verdict && verdict.satisfactory === false) {
          const advisory = {
            issue: verdict.issue || "result judged unsatisfactory",
          ...
          try {
            ...
            if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
              parsed._judge = advisory;
              results[i].content = JSON.stringify(parsed);
              continue;
            }
          } catch { /* non-JSON content: append as text */ }
          results[i].content = `${results[i].content}\n\n[judge] ${advisory.issue}${advisory.suggestion ? ` — ${advisory.suggestion}` : ""}`;
        }
      }
    }
  } catch { /* judging is best-effort */ }
