...
export async function startMigration({ workspaceId, playbookName, repoId } = {}) {
  ...
  migrationIndex.set(migrationId, ws);
  return migration;
}

async function loadMigration(migrationId, workspaceId) {
  const ws = workspaceId ? sanitizeId(workspaceId) : migrationIndex.get(migrationId);
...
export const _internals = { sanitizeId, playbookPath, workspacePath, validatePlaybook };
