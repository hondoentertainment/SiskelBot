...
import { semanticMemoryEnabled, scoreMemoriesSemantic, blendScores } from "./agent-memory-semantic.js";
...
export async function injectMemories({
  ...
  try {
    ...
    // 3. Score and rank. When AGENT_MEMORY_SEMANTIC=1 and embeddings are
    //    available, blend the keyword-based combined score with a cosine
    //    similarity score. Falls back to keyword-only on failure.
    let semanticScores = null;
    if (semanticMemoryEnabled()) {
      semanticScores = await scoreMemoriesSemantic(allMemories, userText);
    }
    const scored = allMemories.map((mem, i) => {
      const s = scoreMemory(mem, userText);
      const combined = semanticScores ? blendScores(semanticScores[i] || 0, s.combined) : s.combined;
      return { ...mem, _scores: { ...s, semantic: semanticScores ? (semanticScores[i] || 0) : null, combined } };
    });
