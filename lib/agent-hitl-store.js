...
import { recordHitlEvent } from "./agent-hitl-audit.js";
...
/** Tracks tokens that were consumed so callers can distinguish 404 vs 409. */
...
export function saveHitlState(state, ttlMs = DEFAULT_TTL_MS) {
  ...
  // Wave-2: durable audit log (AGENT_HITL_AUDIT=1).
  try {
    const info = buildHitlRequestPayload(state, token);
    recordHitlEvent({
      event: "request",
      token,
      approvalId: token,
      sessionId: state && typeof state === "object" ? state.sessionId : undefined,
      userId: state && typeof state === "object" ? state.userId : undefined,
      workspaceId: state && typeof state === "object" ? (state.workspaceId || state.workspace) : undefined,
      toolName: info.tool,
 ...
 * @returns {object | null}
 */
export function isHitlConsumed(token) {
  return consumed.has(String(token || "").trim());
}
...
export function takeHitlState(token, opts) {
  ...
  if (st && opts && typeof opts.decision === "string") {
    ...
    // Wave-2: durable audit log (AGENT_HITL_AUDIT=1).
    try {
      const event = opts.decision === "approve" ? "approve" : opts.decision === "deny" ? "deny" : null;
      if (event) {
        recordHitlEvent({
          event,
          token: id,
          approvalId: id,
          sessionId: st && typeof st === "object" ? st.sessionId : undefined,
          userId: st && typeof st === "object" ? st.userId : undefined,
          workspaceId: st && typeof st === "object" ? (st.workspaceId || st.workspace) : undefined,
        });
      }
    } catch { /* audit is best-effort */ }
  }
