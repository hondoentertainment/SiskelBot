 ...
 * @typedef {import("./types.d.ts").KeyInfo} KeyInfo
 * @typedef {import("./types.d.ts").KeyEntry} KeyEntry
 *
 * Secret rotation: USER_API_KEYS_PREVIOUS is merged into the key map alongside
 * USER_API_KEYS. Keys from the previous set are tagged so that req.usingPreviousKey
 * is set to true when a previous-generation key is used for authentication.
 */
...
/**
 * Parse a single "key:userId:scopes" entry from USER_API_KEYS env var.
 * @param {string} pair - Colon-delimited key entry (e.g. "sk-abc:user1:read,write")
...
/**
 * Lazily load and cache key-to-user mappings from USER_API_KEYS env and users.json.
...
function loadKeyMappings() {
  ...
  // 1b. USER_API_KEYS_PREVIOUS: merged for rotation; tagged as previous
  const envKeysPrev = process.env.USER_API_KEYS_PREVIOUS;
  if (envKeysPrev && typeof envKeysPrev === "string") {
    for (const pair of envKeysPrev.split(",")) {
      const entry = parseEnvKeyEntry(pair);
...
export function resolveKeyInfo(apiKey) {
  ...
  if (entry) return { userId: entry.userId, scopes: entry.scopes, previous: entry.previous || false };
  return null;
}
