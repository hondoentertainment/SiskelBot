...
import { resolveStorageRoot } from "./env-data-root.js";

const ANONYMOUS_USER = "anonymous";
const DEFAULT_SCOPES = ["read", "write"];
const USERS_JSON_PATH = join(resolveStorageRoot(), "users.json");

/** @type {Map<string, { userId: string, scopes: string[] }> | null} key -> { userId, scopes } */
let keyToInfo = null;

/**
 * Parse a single USER_API_KEYS entry like "key:userId" or "key:userId:read,write".
 * @param {string} pair
 * @returns {{ key: string; userId: string; scopes: string[] } | null}
 */
...
/**
 * Load and cache key-to-user mappings from environment and users.json.
 * @returns {Map<string, { userId: string; scopes: string[] }>}
 */
function loadKeyMappings() {
  ...
  // 1b. USER_API_KEYS_PREVIOUS env: merge previous keys for zero-downtime rotation
  // Previous keys do not overwrite current keys (current takes precedence)
  const envPrevKeys = process.env.USER_API_KEYS_PREVIOUS;
  if (envPrevKeys && typeof envPrevKeys === "string") {
    for (const pair of envPrevKeys.split(",")) {
      const entry = parseEnvKeyEntry(pair);
      if (entry && !keyToInfo.has(entry.key)) {
        keyToInfo.set(entry.key, { userId: entry.userId, scopes: entry.scopes, previous: true });
      }
    }
  }
...
export function isAuthConfigured() {
  ...
  const hasOIDC = Boolean(process.env.OIDC_ISSUER && process.env.OIDC_CLIENT_ID && process.env.OIDC_CLIENT_SECRET);
  const hasSAML = Boolean(process.env.SAML_ENTRY_POINT && process.env.SAML_ISSUER);
  return hasGitHub || hasGoogle || hasOIDC || hasSAML;
}
 ...
 * @param {string} apiKey
 * @returns {SiskelBot.KeyInfo | null}
 */
export function resolveKeyInfo(apiKey) {
  ...
  if (entry) return { userId: entry.userId, scopes: entry.scopes, ...(entry.previous ? { previous: true } : {}) };
  return null;
}
 ...
 * @param {import("express").Request & { userId?: string; apiKeyScopes?: string[]; apiKeyId?: string; authenticatedViaDeploymentKey?: boolean; session?: any }} req
...
export function userAuth(req, res, next) {
  ...
  if (info.previous) req.usingPreviousKey = true;
  next();
}
