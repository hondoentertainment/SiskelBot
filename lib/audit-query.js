...
// DATA_DIR is resolved lazily so that tests that override STORAGE_PATH
// after module import still get the correct path.
function dataDir() {
  return process.env.STORAGE_PATH || join(PROJECT_ROOT, "data");
}

function auditLogPath() {
  return join(dataDir(), "execution-audit.json");
}

function localArchiveDir() {
  return join(dataDir(), "audit-archive");
}
