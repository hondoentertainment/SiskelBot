...
import { generateAgentMcpTools, handleAgentMcpToolCall } from "./mcp-agent-tools.js";
...
export async function handleMcpRequest(method, params, ctx = {}) {
  ...
  switch (method) {
    ...
    case "tools/list": {
      let tools = [...MCP_TOOLS];
      if (process.env.EXPOSE_AGENT_MCP_TOOLS === "1") {
        const agentTools = await generateAgentMcpTools(workspace);
    ...
    case "tools/call": {
      ...
      switch (toolName) {
        ...
        default: {
          if (process.env.EXPOSE_AGENT_MCP_TOOLS === "1" && typeof toolName === "string" && toolName.startsWith("agent_")) {
            return handleAgentMcpToolCall(toolName, args, ctx);
          }
          return {
