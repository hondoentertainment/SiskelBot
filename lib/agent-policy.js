...
import { scanToolArgs } from "./safety-probe-gate.js";
 ...
 * @param {string} toolName
 * @param {object} [toolArgs] - tool arguments for prompt-injection scanning when SAFETY_PROBE_GATE=1
 * @returns {{ ok: true } | { ok: false, code: string, reason: string }}
 */
export function checkPolicyBeforeTool(state, toolName, toolArgs) {
  ...
  if (toolArgs !== undefined) {
    const safetyVerdict = scanToolArgs(toolArgs);
    if (!safetyVerdict.safe) {
      recordPolicyDenial("POLICY_PROMPT_INJECTION");
      return {
        ok: false,
        code: "POLICY_PROMPT_INJECTION",
        reason: safetyVerdict.reason || "prompt injection detected in tool args",
      };
    }
  }
