...
import { ADVANCED_META_TOOL_DEFS, runAdvancedMetaTool } from "./agent-tools-advanced.js";
...
const CORE_TOOLS = [
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Save a personal thought for the advice companion journal (note, idea, reminder, moment, or decision). Use when the user wants something remembered for later honest advice.",
      parameters: {
        type: "object",
        properties: {
          ...
          kind: {
            type: "string",
            enum: ["note", "idea", "reminder", "moment", "decision"],
            description: "Thought type (default note)",
          },
          resurfaceAt: {
            type: "string",
            description: "Optional ISO time to resurface this thought",
          },
        },
        required: ["content"],
      },
          ...
          parameters: {
            ...
            properties: {
              ...
              planDag: {
                type: "object",
                description:
                  "Structured plan: e.g. { nodes: [{id}], edges: [{from,to}] } or free-form { steps: [...] }. When AGENT_SESSION_PLAN_DAG_LINT=strict, graph-shaped DAGs are validated (acyclic, edge endpoints exist).",
                additionalProperties: true,
              },
            },
          },
...
const GOOGLE_TOOL_DEFS =
  process.env.AGENT_GOOGLE_WORKSPACE === "1" || googleWorkspaceConfigured()
    ? [
        {
          type: "function",
          function: {
            ...
            description:
              "List recent Gmail messages (optional search query). Requires GOOGLE_WORKSPACE_ACCESS_TOKEN with Gmail readonly scope.",
            parameters: {
              type: "object",
              properties: {
                query: { type: "string", description: "Gmail search query (e.g. is:unread newer_than:7d)" },
                maxResults: { type: "number", description: "Max messages (default 10, max 25)" },
              },
            },
          },
        },
        {
          ...
          function: {
            ...
            description:
              "Send an email via Gmail. Requires human approval (HITL) by default. Needs Gmail send scope.",
            parameters: {
              type: "object",
              properties: {
        ...
        },
        {
          ...
          function: {
            ...
            description: "List upcoming Google Calendar events. Requires Calendar readonly scope.",
            parameters: {
              type: "object",
              properties: {
        ...
        },
        {
          ...
          function: {
            ...
            description:
              "Create a Google Calendar event. Requires human approval (HITL) by default. Needs Calendar write scope.",
            parameters: {
              type: "object",
              properties: {
...
/**
 * Drop tools currently in cooldown for a workspace.
 * @param {Array<{ type: string; function: object }>} tools
 ...
 * @param {{ workspace?: string }} [ctx] - when `ctx.workspace` is set, tools in cooldown are filtered out
 * @returns {Array<{ type: string; function: object }>}
 */
 ...
 * When `ctx.workspace` is provided, also drops tools in cooldown for that workspace.
 * @param {Array<{ type: string; function: object }>} tools
 ...
 * @param {{ workspace?: string }} [ctx] - when `ctx.workspace` set, tools in cooldown are filtered out
 * @returns {Array<{ type: string; function: object }>}
 */
...
async function runToolCore(name, args, ctx) {
  ...
  switch (name) {
    ...
    case "capture_thought": {
      const { captureThought } = await import("./advice-companion.js");
      const uid = sanitizeUserId(ctx.workspaceUserId || "anonymous");
      try {
        const thought = await captureThought(uid, workspace, {
          content: args?.content,
          kind: args?.kind,
          resurfaceAt: args?.resurfaceAt,
        });
        return { content: JSON.stringify({ ok: true, thought }) };
      } catch (err) {
    ...
    case "gmail_list_messages": {
      const r = await gmailListMessages({
        query: args?.query,
        maxResults: args?.maxResults,
      });
      return { content: JSON.stringify(r), ok: r.ok !== false };
    }

    case "gmail_send_message": {
      const r = await gmailSendMessage({
        to: args?.to,
    ...
    case "code_execute": {
      ...
      const sandboxMode = (process.env.AGENT_CODE_SANDBOX || "").trim().toLowerCase();
      if (sandboxMode && sandboxMode !== "0" && sandboxMode !== "off") {
        try {
          const { runCodeInSandbox } = await import("./agent-sandbox.js");
          const runtime =
            sandboxMode === "docker" ? "docker" : sandboxMode === "mock" ? "mock" : "mock";
          const out = await runCodeInSandbox(code, { timeoutMs, runtime });
          return {
            content: JSON.stringify({
              ok: out.ok === true,
              output: out.output || "",
              ...
              error: out.error,
              sandbox: { runtime: out.runtime, sandboxId: out.sandboxId, timedOut: out.timedOut },
            }),
          };
        } catch (err) {
          return {
    ...
    default: {
      const advanced = await runAdvancedMetaTool(name, args, ctx, runTool);
      if (advanced) return advanced;
      return { content: JSON.stringify({ ok: false, error: `Unknown tool: ${name}` }), ok: false };
    }
  }
}
