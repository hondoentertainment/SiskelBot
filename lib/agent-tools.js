...
import { searchRepo as repoRagSearch } from "./repo-rag.js";
import { planRefactor, previewRefactor } from "./refactor-agent.js";
    ...
    ? [
        {
          ...
          function: {
            name: "search_repo",
            description:
              "Search indexed repository source files for a query (keyword search over the per-workspace repo-rag inverted index). Returns top-N file/snippet matches with repoId, path, score, and matching line numbers. Use to locate code relevant to a refactor or bug before proposing a patch. Requires AGENT_CODEGEN_TOOLS=1.",
            parameters: {
              type: "object",
              properties: {
                query: { type: "string", description: "Keyword query (tokenized same way as the index)" },
                limit: { type: "number", description: "Maximum number of file hits (default 20, max 500)" },
              },
              required: ["query"],
            },
          },
        },
        {
          ...
          function: {
            name: "propose_patch",
            description:
              "Propose a dry-run refactor patch for a set of files. Accepts an intent object describing the transform (type: 'rename' | 'replace', from, to) and a files array of { path, content } pairs. Returns the proposed diff including per-file before/after snapshots and change counts. This tool NEVER writes to disk — applying a plan requires the HTTP route (POST /refactor/plans/:id/apply) with writeToDisk=true AND WORKSPACE_FILE_TOOLS=1. Requires AGENT_CODEGEN_TOOLS=1.",
            parameters: {
              type: "object",
              properties: {
                files: {
                  type: "array",
                  description: "Files to transform, each { path: string, content: string }",
                  items: {
                    type: "object",
                    properties: {
                      path: { type: "string" },
                ...
                intent: {
                  type: "object",
                  description: "Transform description: { type: 'rename' | 'replace', from: string, to: string }",
                  properties: {
                    type: { type: "string" },
                    from: { type: "string" },
                    to: { type: "string" },
          ...
          ? [
              {
                ...
                function: {
                  name: "run_tests",
                  description:
                    "Run the project's Node.js test suite (node --test) as a child process with a 60-second timeout. Accepts an optional glob pattern to narrow which test files are executed. Returns pass/fail counts and the first failure stack (truncated). Requires BOTH AGENT_CODEGEN_TOOLS=1 AND WORKSPACE_FILE_TOOLS=1.",
                  parameters: {
                    type: "object",
                    properties: {
                      pattern: {
                        type: "string",
                        description: "Optional test file pattern (e.g. 'tests/agent-tools-*.test.js'). When omitted the runner uses its default discovery.",
                      },
                    },
                  },
                },
...
async function runToolCore(name, args, ctx) {
  ...
  switch (name) {
    ...
    case "search_repo": {
      if (process.env.AGENT_CODEGEN_TOOLS !== "1") {
        return { content: JSON.stringify({ ok: false, error: "AGENT_CODEGEN_TOOLS is not enabled" }) };
      }
      const query = typeof args?.query === "string" ? args.query.trim() : "";
      if (!query) {
      ...
      try {
        const result = await repoRagSearch({ workspaceId: workspace, query, limit });
        return {
          content: JSON.stringify({
            ok: true,
            query,
    ...
    case "propose_patch": {
      ...
      if (!files || files.length === 0) {
        return { content: JSON.stringify({ ok: false, error: "files array is required and must not be empty" }) };
      }
      if (!intent || typeof intent !== "object") {
        return { content: JSON.stringify({ ok: false, error: "intent object is required" }) };
      }
      const transform = {
        type: typeof intent.type === "string" ? intent.type : "",
        from: typeof intent.from === "string" ? intent.from : "",
        to: typeof intent.to === "string" ? intent.to : "",
      };
      try {
        const plan = await planRefactor({ workspaceId: workspace, files, transform });
        // Dry-run only: never invoke applyRefactor from this tool.
        const preview = await previewRefactor(plan.planId, workspace);
        return {
          content: JSON.stringify({
            ok: true,
            dryRun: true,
            ...
            note: "Apply requires POST /refactor/plans/:id/apply with writeToDisk=true AND WORKSPACE_FILE_TOOLS=1.",
          }),
          ok: true,
        };
      } catch (err) {
    ...
    case "run_tests": {
      ...
      if (process.env.WORKSPACE_FILE_TOOLS !== "1") {
        return { content: JSON.stringify({ ok: false, error: "WORKSPACE_FILE_TOOLS is not enabled" }) };
      }
      const pattern = typeof args?.pattern === "string" ? args.pattern.trim() : "";
      if (pattern && /[;&|`$<>]/.test(pattern)) {
        return { content: JSON.stringify({ ok: false, error: "pattern contains unsafe characters", code: "UNSAFE_PATTERN" }) };
      }
      const timeoutMs = Math.min(60_000, Math.max(1_000, Number(process.env.AGENT_RUN_TESTS_TIMEOUT_MS) || 60_000));
      const cwd = (typeof ctx.workspaceFilesystemRoot === "string" && ctx.workspaceFilesystemRoot.trim())
        || (process.env.WORKSPACE_ROOT || "").trim()
        || process.cwd();
      const execArgv = ["--test"];
      if (pattern) execArgv.push(pattern);
      return await new Promise((resolve) => {
        ...
        let child;
        try {
          child = spawn(process.execPath, execArgv, {
            cwd,
            env: { ...process.env },
            stdio: ["ignore", "pipe", "pipe"],
          });
        } catch (err) {
          resolve({ content: JSON.stringify({ ok: false, error: String(err?.message || err), code: "SPAWN_FAILED" }) });
          return;
        }
        ...
        child.stdout.on("data", (chunk) => {
          ...
          if (stdout.length < 200_000) stdout += s;
        });
        child.stderr.on("data", (chunk) => {
          const s = chunk.toString("utf8");
          if (stderr.length < 100_000) stderr += s;
        });
        child.on("error", (err) => {
          if (done) return;
          done = true;
          clearTimeout(timer);
          resolve({ content: JSON.stringify({ ok: false, error: String(err?.message || err), code: "SPAWN_ERROR" }) });
        });
        child.on("close", (code) => {
          if (done) return;
          done = true;
          ...
          const failIdx = stdout.indexOf("not ok");
          if (failIdx >= 0) {
            firstFailure = stdout.slice(failIdx, failIdx + 2000);
          }
          resolve({
            content: JSON.stringify({
              ok: !timedOut && fail === 0 && code === 0,
              timedOut,
              exitCode: code,
              pass,
              fail,
