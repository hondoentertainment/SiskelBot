...
import {
  ...
  workspaceMoveFile,
  workspaceListDir,
} from "./workspace-fs-tools.js";
import { searchCodebase, findDefinition, findReferences } from "./codebase-search.js";
import { hydratePlaybook, incrementUsage } from "./playbooks.js";
...
export const TOOLS = [
    ...
    ? [
        ...
        },
        {
          ...
          function: {
            ...
            description:
              "Write UTF-8 text to a file in the workspace (creates parent dirs automatically). Returns { ok, path, byteLength }.",
            parameters: {
              type: "object",
              properties: {
                path: { type: "string", description: "Relative file path within the workspace root" },
        ...
        },
        {
          ...
          function: {
            ...
            description:
              "Apply a unified diff to a file in the workspace. Returns { ok, path, linesAdded, linesRemoved }.",
            parameters: {
              type: "object",
              properties: {
                path: { type: "string", description: "Relative file path within the workspace root" },
                patch: { type: "string", description: "Unified diff string (output of git diff or similar)" },
              },
              required: ["path", "patch"],
            },
          },
        },
        {
          ...
          function: {
            ...
            description: "Delete a file or empty directory from the workspace. Returns { ok, path }.",
            parameters: {
              type: "object",
              properties: {
        ...
        },
        {
          ...
          function: {
            ...
            description: "Create a directory (and all parents) in the workspace. Returns { ok, path }.",
            parameters: {
              type: "object",
              properties: {
        ...
        },
        {
          ...
          function: {
            ...
            description: "Move or rename a file within the workspace. Returns { ok, from, to }.",
            parameters: {
              type: "object",
              properties: {
        ...
        },
        {
          ...
          function: {
            ...
            description: "List files and directories in a workspace directory. Returns { ok, entries, truncated }.",
            parameters: {
              type: "object",
              properties: {
        ...
        },
        {
          ...
          function: {
            ...
            description:
              "Ask a natural-language question about the codebase and get a synthesized answer with file:line citations. Use instead of raw text search when you need an explanation.",
            parameters: {
              type: "object",
              properties: {
                query: { type: "string", description: "Natural-language question about the codebase" },
                model: { type: "string", description: "LLM model override for synthesis (optional)" },
              },
              required: ["query"],
            },
          },
        },
        {
          ...
          function: {
            ...
            description: "Find where a symbol (function, class, variable) is defined in the workspace. Returns file:line locations.",
            parameters: {
              type: "object",
              properties: {
        ...
        },
        {
          ...
          function: {
            ...
            description: "Find all usages of a symbol across the workspace. Returns file:line locations.",
            parameters: {
              type: "object",
              properties: {
    ...
    : []),
  {
    ...
    function: {
      name: "use_playbook",
      description:
        "Look up a saved playbook by ID and hydrate it with parameter values. Returns the task prompt and system instructions to use for the current session.",
      parameters: {
        type: "object",
        properties: {
          playbookId: { type: "string", description: "Playbook ID to use" },
          params: { type: "object", description: "Parameter values to substitute into the playbook template", additionalProperties: true },
          workspaceId: { type: "string", description: "Workspace that owns the playbook (or '_gallery' for community playbooks)" },
        },
        required: ["playbookId"],
      },
    },
...
/**
 * Copy parsed `{ ok }` from JSON tool payloads onto the wrapper when missing — keeps agent-loop metrics honest.
 * @param {SiskelBot.ToolResult | Record<string, unknown>} result
 */
function coalesceToolOuterOk(result) {
  ...
  switch (name) {
    ...
    case "workspace_write_file": {
      ...
      if (typeof content !== "string") {
        return { content: JSON.stringify({ ok: false, error: "content is required" }), ok: false };
      }
      const result = await workspaceWriteFile(ctx, filePath.trim(), content, { createDirs: args?.createDirs !== false });
      return { content: JSON.stringify(result), ok: !!result.ok };
    }

    case "workspace_patch_file": {
      if (process.env.WORKSPACE_FILE_TOOLS !== "1") {
        return { content: JSON.stringify({ ok: false, error: "WORKSPACE_FILE_TOOLS is not enabled" }), ok: false };
      }
      const filePath = args?.path;
      ...
      if (typeof patch !== "string" || !patch.trim()) {
        return { content: JSON.stringify({ ok: false, error: "patch is required" }), ok: false };
      }
      const result = await workspacePatchFile(ctx, filePath.trim(), patch);
      return { content: JSON.stringify(result), ok: !!result.ok };
    }
    ...
    case "workspace_delete_path": {
      ...
      const result = await workspaceDeletePath(ctx, filePath.trim());
      return { content: JSON.stringify(result), ok: !!result.ok };
    }
    ...
    case "workspace_create_dir": {
      ...
      const result = await workspaceCreateDir(ctx, dirPath.trim());
      return { content: JSON.stringify(result), ok: !!result.ok };
    }
    ...
    case "workspace_move_file": {
      ...
      if (typeof fromPath !== "string" || !fromPath.trim()) {
        return { content: JSON.stringify({ ok: false, error: "from is required" }), ok: false };
      }
      if (typeof toPath !== "string" || !toPath.trim()) {
        return { content: JSON.stringify({ ok: false, error: "to is required" }), ok: false };
      }
      const result = await workspaceMoveFile(ctx, fromPath.trim(), toPath.trim());
      return { content: JSON.stringify(result), ok: !!result.ok };
    }
    ...
    case "workspace_list_dir": {
      ...
      const dirPath = args?.path || ".";
      const result = await workspaceListDir(ctx, dirPath);
      return { content: JSON.stringify(result), ok: !!result.ok };
    }
    ...
    case "codebase_search": {
      ...
      const query = args?.query;
      if (typeof query !== "string" || !query.trim()) {
        return { content: JSON.stringify({ ok: false, error: "query is required" }), ok: false };
      }
      try {
        const result = await searchCodebase(workspace, query.trim(), ctx, ctx.llmClient, { model: args?.model });
        return { content: JSON.stringify(result), ok: true };
      } catch (e) {
        return { content: JSON.stringify({ ok: false, error: String(e?.message || e) }), ok: false };
      }
    }
    ...
    case "find_definition": {
      ...
      if (typeof symbol !== "string" || !symbol.trim()) {
        return { content: JSON.stringify({ ok: false, error: "symbol is required" }), ok: false };
      }
      try {
        const result = await findDefinition(workspace, symbol.trim(), ctx);
        return { content: JSON.stringify(result), ok: true };
      } catch (e) {
        return { content: JSON.stringify({ ok: false, error: String(e?.message || e) }), ok: false };
      }
    ...
    case "find_references": {
      ...
      try {
        const result = await findReferences(workspace, symbol.trim(), ctx);
        return { content: JSON.stringify(result), ok: true };
      } catch (e) {
        return { content: JSON.stringify({ ok: false, error: String(e?.message || e) }), ok: false };
      }
    ...
    case "use_playbook": {
      ...
      if (typeof playbookId !== "string" || !playbookId.trim()) {
        return { content: JSON.stringify({ ok: false, error: "playbookId is required" }), ok: false };
      }
      const wsId = typeof args?.workspaceId === "string" ? args.workspaceId : workspace;
      const params = args?.params && typeof args.params === "object" ? args.params : {};
      try {
        const result = await hydratePlaybook(playbookId.trim(), params, wsId);
        if (result) await incrementUsage(playbookId.trim(), wsId);
        return { content: JSON.stringify({ ok: !!result, playbook: result }), ok: !!result };
      } catch (e) {
        return { content: JSON.stringify({ ok: false, error: String(e?.message || e) }), ok: false };
      }
    }
