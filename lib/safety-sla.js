...
import { listSamples } from "./eval-history-store.js";
...
/**
 * Import binary outcomes from eval-history safety samples (passRate proxy).
 * Each sample: prediction = passRate >= passThreshold, actual = passRate >= actualThreshold.
 *
 * @param {object} opts
 * @param {string} opts.workspaceId
 * @param {string} [opts.classifierId]
 * @param {number} [opts.sinceTs]
 * @param {number} [opts.limit]
 * @param {number} [opts.passThreshold=0.8]
 * @param {number} [opts.actualThreshold=0.5]
 * @returns {Promise<{ imported: number, skipped: number, classifierId: string }>}
 */
export async function seedFromEvalHistory({
  workspaceId,
  classifierId = "eval-safety",
  sinceTs,
  limit = 500,
  passThreshold = 0.8,
  actualThreshold = 0.5,
  ...
  const id = sanitizeId(classifierId);
  const samples = await listSamples(workspaceId, {
    kind: "safety",
  ...
  for (const sample of samples) {
    const passRate = sample.metrics?.passRate;
    ...
    await recordClassification({
      classifierId: id,
      prediction: passRate >= passThreshold,
      actual: passRate >= actualThreshold,
      timestamp: sample.ts,
    });
    imported += 1;
  }
  return { imported, skipped, classifierId: id };
}
