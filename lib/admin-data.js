/**
 * Phase 25: Admin dashboard data aggregation.
 * Collects users, workspaces, usage, quotas, health, and audit for admin UI.
 * Phase 68: OAuth users and audit log from json-path-store (Postgres / SQLite / file).
 */
import { readdirSync, existsSync, statSync } from "fs";
import { join } from "path";
import * as storage from "./storage.js";
import { readJsonPath, getDataDir } from "./json-path-store.js";
import { loadApiKeys } from "./api-keys.js";

const DATA_DIR = process.env.STORAGE_PATH || join(process.cwd(), "data");
const USERS_JSON_PATH = join(DATA_DIR, "users.json");
const AUDIT_LOG_PATH = join(DATA_DIR, "execution-audit.json");
const USERS_DIR = join(DATA_DIR, "users");

/**
 * List all users from data/users dir, oauth-users.json, users.json, and admin API keys store.
 * @returns {Promise<Array<{ userId: string, source: string, provider?: string }>>}
 */
export async function listAllUsers() {
  const seen = new Set();
  const result = [];

  // 1. From data/users/ directories
  if (existsSync(USERS_DIR)) {
    for (const name of readdirSync(USERS_DIR)) {
      if (name.startsWith(".")) continue;
      const stat = statSync(join(USERS_DIR, name));
      if (stat.isDirectory() && !seen.has(name)) {
        seen.add(name);
        result.push({ userId: name, source: "storage" });
      }
    }
  }

  // 2. From oauth-users.json (durable store path)
  try {
    const oauthPath = join(getDataDir(), "oauth-users.json");
    const data = await readJsonPath(oauthPath, { users: [] });
    const users = Array.isArray(data.users) ? data.users : [];
    for (const u of users) {
      const id = u?.userId;
      if (id && !seen.has(id)) {
        seen.add(id);
        result.push({ userId: id, source: "oauth", provider: u?.provider });
      }
    }
  } catch (e) {
    console.warn("[admin-data] Failed to load oauth-users:", e.message);
  }

  // 3. From users.json (API key mappings) — durable store
  try {
    const data = await readJsonPath(USERS_JSON_PATH, {});
    const userIds = [];
    if (Array.isArray(data.users)) {
      for (const u of data.users) {
        if (u?.userId) userIds.push(u.userId);
      }
    } else if (data && typeof data === "object") {
      for (const v of Object.values(data)) {
        if (typeof v === "string") userIds.push(v);
      }
    }
    for (const id of userIds) {
      if (id && !seen.has(id)) {
        seen.add(id);
        result.push({ userId: id, source: "api-keys" });
      }
    }
  } catch (e) {
    console.warn("[admin-data] Failed to load users.json:", e.message);
  }

  // 4. From data/api-keys.json (admin-managed keys)
  try {
    for (const k of loadApiKeys()) {
      const id = k?.userId;
      if (id && !seen.has(id)) {
        seen.add(id);
        result.push({ userId: id, source: "user-api-keys" });
      }
    }
  } catch (e) {
    console.warn("[admin-data] Failed to load api-keys store:", e.message);
  }

  return result;
}

/**
 * List all workspaces across users.
 * @returns {Array<{ userId: string, workspace: object }>}
 */
export async function listAllWorkspaces() {
  const users = await listAllUsers();
  const result = [];
  for (const { userId } of users) {
    const workspaces = await storage.listWorkspaces(userId);
    for (const ws of workspaces) {
      result.push({ userId, workspace: ws });
    }
  }
  return result;
}

/**
 * Get recent audit log entries.
 * @param {number} limit
 * @returns {Array<object>}
 */
export async function getRecentAuditLog(limit = 50) {
  try {
    const entries = await readJsonPath(AUDIT_LOG_PATH, []);
    const arr = Array.isArray(entries) ? entries : [];
    return arr.slice(-limit).reverse();
  } catch (e) {
    console.warn("[admin-data] Failed to load audit log:", e.message);
  }
  return [];
}
