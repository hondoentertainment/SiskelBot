...
import { redisKvLoad, redisKvSave, redisKvEnabled } from "./storage-redis-kv.js";
...
async function loadRaw(filePath) {
  ...
  if (redisKvEnabled()) {
    const fromRedis = await redisKvLoad(filePath);
    if (fromRedis != null && typeof fromRedis === "object") return fromRedis;
    return { _version: STORAGE_VERSION, items: [] };
  }
  if (sqliteKvEnabled()) {
...
async function saveRaw(filePath, data) {
  ...
  if (redisKvEnabled()) {
    const ok = await redisKvSave(filePath, data);
    if (!ok) console.error("[storage] Redis save failed for", filePath);
    return;
  }
...
async function migrateLegacyIfNeeded(type, userId, workspaceId) {
  if (postgresKvEnabled() || redisKvEnabled()) return;
  const uid = sanitizeUserId(userId);
  const ws = sanitizeWorkspace(workspaceId);
