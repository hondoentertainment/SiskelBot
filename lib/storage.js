 ...
 *
 * @typedef {import("./types.d.ts").StorageType} StorageType
 * @typedef {import("./types.d.ts").StorageItem} StorageItem
 * @typedef {import("./types.d.ts").StorageEnvelope} StorageEnvelope
...
/**
 * Resolve the root data directory (from STORAGE_PATH env or ./data).
 * @returns {string}
 */
function getDataDir() {
...
/**
 * Sanitize a user ID to a safe filesystem-compatible string; falls back to "anonymous".
 * @param {string} uid
 * @returns {string}
 */
...
/**
 * Sanitize a workspace ID to a safe filesystem-compatible string; falls back to "default".
 * @param {string} ws
 * @returns {string}
 */
...
/**
 * Build the scoped file path for a storage type (per-user, per-workspace).
...
/**
 * Build the legacy (non-scoped) file path for migration.
 * @param {StorageType} type
 * @returns {string}
 */
...
/**
 * Read a legacy JSON file synchronously (no KV backend).
 * @param {string} filePath
 * @returns {StorageEnvelope}
 */
...
/**
 * Load a storage envelope from the active backend (Postgres > SQLite > JSON file).
 * @param {string} filePath - Logical key (file path used as KV key in DB backends)
 * @returns {Promise<StorageEnvelope>}
 */
...
/**
 * Persist a storage envelope to the active backend (Postgres > SQLite > JSON file).
 * @param {string} filePath - Logical key
 * @param {StorageEnvelope} data
...
/**
 * Serialize async operations on the same filePath to prevent concurrent writes.
 * @template T
 * @param {string} filePath
 * @param {() => Promise<T>} fn
 * @returns {Promise<T>}
...
/**
 * One-time migration of legacy (non-scoped) data to the scoped path.
 * @param {StorageType} type
 * @param {string} userId
 * @param {string} workspaceId
 * @returns {Promise<void>}
...
/**
 * Path to user's workspaces.json metadata file.
 * @param {string} userId
 * @returns {string}
 */
...
/**
 * List all items of a given storage type for a user/workspace.
 * @param {StorageType} type
...
/**
 * Add one or more items. When merge=true, updates existing items by ID.
 * @param {StorageType} type
 * @param {StorageItem | StorageItem[]} itemOrItems
 * @param {string} [workspace="default"]
 * @param {boolean} [merge=false]
 * @param {string} [userId="anonymous"]
 * @returns {Promise<StorageItem | StorageItem[]>} - Single item when not merging, full list when merging
 */
...
/**
 * Merge client items with server items by ID (upsert semantics).
 * @param {StorageType} type
 * @param {StorageItem[]} clientItems
 * @param {string} [workspace="default"]
 * @param {string} [userId="anonymous"]
 * @returns {Promise<StorageItem[]>}
 */
...
/**
 * Replace all items for a storage type (destructive overwrite).
 * @param {StorageType} type
 * @param {StorageItem[]} newItems
 * @param {string} [workspace="default"]
 * @param {string} [userId="anonymous"]
 * @returns {Promise<StorageItem[]>}
 */
...
/**
 * Partial update of a single item by ID. Returns null if not found.
 * @param {StorageType} type
 * @param {string} id
 * @param {Partial<StorageItem>} updates
 * @param {string} [workspace="default"]
 * @param {string} [userId="anonymous"]
 * @returns {Promise<StorageItem | null>}
 */
...
/**
 * List all workspaces for a user, including team workspaces they belong to.
 * @param {string} [userId="anonymous"]
...
/**
 * Create a new workspace and persist it to the user's workspaces.json.
 * @param {string} userId
 * @param {string} name - Display name for the workspace
 * @param {"personal" | "team"} [type="personal"]
 * @returns {Promise<Workspace>}
 */
...
/** @type {typeof mergeSync} */
export async function mergeItems(type, workspace, items, userId = "anonymous") {
  return mergeSync(type, items, workspace, userId);
}
/**
 * Load an item, apply a transform function, and save it back.
 * @param {StorageType} type
 * @param {string} id
 * @param {string} workspace
 * @param {(item: StorageItem) => Partial<StorageItem>} fn - Transform function
 * @param {string} [userId="anonymous"]
 * @returns {Promise<StorageItem | null>}
 */
