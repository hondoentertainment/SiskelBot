...
export const mockRuntime = {
  ...
  async exec(containerId, command, options = {}) {
    ...
    if (typeof command === "string" && command.startsWith("__siskel_js__:")) {
      const entry = runMockJsPayload(command, options.timeoutMs || timeoutSec * 1000);
      mockLogs.get(containerId).push(entry);
      return entry;
    }
...
/**
 * Execute a base64 JS payload inside the mock sandbox via node:vm (isolated context).
 * @param {string} command
 * @param {number} timeoutMs
 */
function runMockJsPayload(command, timeoutMs) {
  const b64 = command.slice("__siskel_js__:".length);
  let code = "";
  try {
  ...
  try {
    ...
    const result = script.runInContext(ctx, {
      timeout: Math.min(Math.max(100, Number(timeoutMs) || 5000), 30_000),
    });
    return {
      command,
      stdout: logs.join("\n"),
    ...
    const timedOut = /Script execution timed out/i.test(String(err?.message || ""));
    return {
      command,
      stdout: logs.join("\n"),
...
/**
 * Run untrusted JS in a short-lived sandbox (mock by default; docker when registered).
 * Used by agent `code_execute` when AGENT_CODE_SANDBOX is set.
 *
 * @param {string} code
 * @param {{ timeoutMs?: number, runtime?: string }} [opts]
 */
export async function runCodeInSandbox(code, opts = {}) {
  const timeoutMs = Math.min(Math.max(100, Number(opts.timeoutMs) || 5000), 30_000);
  let runtime = typeof opts.runtime === "string" ? opts.runtime : "mock";
  if (runtime === "docker") {
    try {
    ...
    if (!runtimes.has("docker")) runtime = "mock";
  }
  if (!runtimes.has(runtime)) runtime = "mock";

  const sb = await createSandbox("node-alpine", {
    runtime,
    limits: {
  ...
  try {
    const payload = Buffer.from(String(code || ""), "utf8").toString("base64");
    const execResult = await execInSandbox(sb.sandboxId, `__siskel_js__:${payload}`, {
      timeoutSec: Math.max(1, Math.ceil(timeoutMs / 1000)),
      timeoutMs,
    });
    return {
      ok: execResult.exitCode === 0 && !execResult.timedOut,
      output: execResult.stdout || "",
      result: execResult.result !== undefined ? execResult.result : null,
      error: execResult.exitCode === 0 ? undefined : execResult.stderr || "sandbox execution failed",
    ...
    try {
      await removeSandbox(sb.sandboxId);
    } catch {
      /* cleanup best-effort */
    }
  }
...
export async function execInSandbox(sandboxId, command, options = {}) {
  ...
  return {
    ...
    result: Object.prototype.hasOwnProperty.call(result, "result") ? result.result : undefined,
  };
}
