...
/**
 * Surface scheduled agents that are overdue — their next run is in the past by
 * more than a threshold, i.e. the cadence has stalled (execution disabled or
 * stuck). A temporal/condition-based signal.
 */
export async function overdueScheduledAgentSignals(workspaceId, opts = {}) {
  const overdueMs = Math.max(60_000, Number(opts.overdueMs) || 3_600_000); // default 1h
  const signals = [];
  try {
    const agents = await listScheduledAgents(workspaceId);
    for (const agent of agents) {
      if (!agent.enabled || !agent.nextRun) continue;
      const due = Date.parse(agent.nextRun);
      if (Number.isNaN(due)) continue;
      const lateBy = Date.now() - due;
      if (lateBy >= overdueMs) {
        const hours = Math.floor(lateBy / 3_600_000);
        signals.push({
          kind: "scheduled_agent_overdue",
          summary: `Scheduled agent "${agent.name}" is overdue by ~${hours}h`,
          detail: `Next run was due ${agent.nextRun}; it has not executed.`,
          severity: hours >= 24 ? 4 : 3,
          fingerprint: fingerprintOf(["overdue", agent.id, String(hours)]),
          ref: { type: "scheduled_agent", id: agent.id },
        });
      }
    }
  } catch (e) {
    log.warn("overdueScheduledAgentSignals error", { error: e.message });
  }
  return signals;
}

export const defaultSignalProviders = [
  scheduledAgentFailureSignals,
  overdueScheduledAgentSignals,
  memoryObservationSignals,
];
...
function heuristicProposals(signals) {
  const byCategory = {
    ...
    scheduled_agent_overdue: "fix",
    memory_observation: "investigate",
  };
  const titleFor = (s) => {
    if (s.kind === "scheduled_agent_failure") return "Repair failing scheduled agent";
    if (s.kind === "scheduled_agent_overdue") return "Restart overdue scheduled agent";
    return `Investigate: ${s.summary.slice(0, 60)}`;
  };
  const actionFor = (s) => {
    if (s.kind === "scheduled_agent_failure") return "Review the agent's recent runs and fix the underlying error, or pause it.";
    if (s.kind === "scheduled_agent_overdue") return "Check that execution is enabled and the scheduler leader is healthy.";
    return "Review this observation and decide whether it needs follow-up.";
  };
  return signals
