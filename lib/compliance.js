...
/** ISO/IEC 27001:2022 Annex A controls mapped for operator reporting. */
export const ISO27001_CONTROLS = {
  A_5_1: {
    name: "Policies for information security",
    description: "Information security policy set and communication",
  },
  A_5_15: {
    name: "Access control",
    description: "Rules to limit access to information and assets",
  },
  A_5_24: {
    name: "Information security incident management planning",
    description: "Prepare and respond to security incidents",
  },
  A_5_34: {
  ...
  A_8_2: {
    ...
    description: "Restrict and review privileged access",
  },
  A_8_10: {
    name: "Information deletion",
    description: "Secure deletion when no longer required",
  },
  A_8_24: {
    name: "Use of cryptography",
    description: "Cryptographic controls for confidentiality/integrity",
  },
};
...
export function mapAuditEventToControls(auditEvent) {
  ...
  if (!action) {
    return { soc2: [], hipaa: [], gdpr: [], iso27001: [] };
  }
  ...
  if (soc2.has("CC6_1") || soc2.has("CC6_3")) iso27001.add("A_5_15");
  if (soc2.has("CC6_2") || soc2.has("CC6_3")) iso27001.add("A_8_2");
  if (soc2.has("CC6_6") || soc2.has("CC6_7")) iso27001.add("A_8_24");
  if (soc2.has("CC7_1") || soc2.has("CC7_2")) iso27001.add("A_5_24");
  if (soc2.has("CC8_1")) iso27001.add("A_5_1");
  if (gdpr.has("ART_17") || gdpr.has("ART_5")) iso27001.add("A_8_10");
  if (gdpr.has("ART_15") || gdpr.has("ART_32") || gdpr.has("ART_25")) iso27001.add("A_5_34");
...
/**
 * Generate an ISO/IEC 27001 Annex A report for the given period.
 */
export async function generateISO27001Report(period = {}) {
  const normalized = normalizePeriod(period);
  const entries = await fetchAuditForPeriod(normalized);
  return buildFrameworkReport("ISO27001", ISO27001_CONTROLS, "iso27001", normalized, entries);
}
...
export async function getComplianceDashboard(period = {}) {
  const [soc2, hipaa, gdpr, iso27001] = await Promise.all([
    generateSOC2Report(period),
    generateHIPAAReport(period),
    generateGDPRReport(period),
    generateISO27001Report(period),
  ]);
