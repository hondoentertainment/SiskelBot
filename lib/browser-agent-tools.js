...
/** When set, browser tools pause for HITL if the URL host is not in workspace browserAllowedHosts. */
export function browserNewDomainHitlEnabled() {
  return process.env.AGENT_BROWSER_HITL_NEW_DOMAINS === "1";
}

/**
 * Extract hostname from a URL string (empty string if invalid).
 * @param {string} urlString
 */
export function browserUrlHostname(urlString) {
  try {
    return new URL(String(urlString || "").trim()).hostname.toLowerCase();
  } catch {
    return "";
  }
}
...
/**
 * Decide whether a browser navigation should pause for human approval (new domain).
 * Globally allowlisted URLs whose host is missing from workspace browserAllowedHosts
 * require HITL when AGENT_BROWSER_HITL_NEW_DOMAINS=1. If workspace hosts are empty,
 * every globally allowlisted host is treated as "new".
 *
 * @param {string} urlString
 * @param {string[]} [workspaceBrowserHosts]
 * @returns {{ needsHitl: boolean, host?: string, reason?: string }}
 */
export function needsBrowserDomainHitl(urlString, workspaceBrowserHosts) {
  if (!browserNewDomainHitlEnabled()) return { needsHitl: false };
  const entries = parseEnvBrowserAllowlistEntries();
  if (!entries.length) return { needsHitl: false, reason: "no_global_allowlist" };
  const url = String(urlString || "").trim();
  if (!url || !isUrlAllowedForKnowledge(url, entries)) {
    return { needsHitl: false, reason: "not_globally_allowlisted" };
  }
  const host = browserUrlHostname(url);
  if (!host) return { needsHitl: false, reason: "invalid_url" };
  const ws = normalizeWorkspaceBrowserHosts(workspaceBrowserHosts);
  if (ws.length && isUrlAllowedForKnowledge(url, ws)) {
    return { needsHitl: false, host, reason: "workspace_approved" };
  }
  return {
    needsHitl: true,
    host,
    reason: ws.length ? "host_not_in_workspace_allowlist" : "no_workspace_allowlist",
  };
}
