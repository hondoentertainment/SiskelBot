...
async function getConversationStats(days, opts) {
  ...
  const wsList = await storage.listWorkspaces(userId);
  const workspaces = opts.workspace ? [opts.workspace] : ["default", ...wsList.map((w) => w.id).filter((id) => id !== "default")];
  let totalConvs = 0;
  let totalMessages = 0;

  for (const ws of workspaces) {
    try {
      const items = await storage.list("conversations", ws, userId);
