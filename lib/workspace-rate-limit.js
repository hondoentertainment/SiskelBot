...
function extractWorkspaceId(req) {
  const fromParam = req.params?.id || req.query?.workspace || req.body?.workspace;
  if (fromParam) return fromParam;
  if (req.workspace) return req.workspace;
  const hdr = req.headers?.["x-workspace-id"];
  if (typeof hdr === "string" && hdr.trim()) return hdr.trim();
  const fromAgent = req.body?.agentOptions?.workspace;
  if (typeof fromAgent === "string" && fromAgent.trim()) return fromAgent.trim();
  return null;
}
