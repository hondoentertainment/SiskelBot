 ...
 * @param {string[]} [params.alternatives] - optional list of alternative tool names to suggest
 * @returns {string}
 */
export function buildReplanMessage({ toolName, failure, history, attempt, maxAttempts, alternatives }) {
  const parts = [];
  ...
  if (Array.isArray(alternatives) && alternatives.length > 0) {
    parts.push(`Consider these alternatives instead: ${alternatives.slice(0, 3).join(", ")}.`);
  }
 ...
 * @param {number} [params.maxAttempts]
 * @param {string[]} [params.alternatives]
