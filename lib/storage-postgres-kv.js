...
export async function postgresKvLoad(filePath) {
  try {
    ...
    if (!pool) return null;
    const r = await instrumentedQuery(pool, "SELECT data FROM storage_kv WHERE path = $1", [filePath]);
    const row = r.rows?.[0];
...
export async function postgresKvExportAll() {
  const pool = await ensurePool();
  if (!pool) return [];
  const r = await instrumentedQuery(pool, "SELECT path, data FROM storage_kv ORDER BY path");
...
export async function postgresKvImportSnapshot(entries, opts = {}) {
  ...
  try {
    ...
    if (replaceAll) {
      await instrumentedQuery(c, "DELETE FROM storage_kv");
    }
