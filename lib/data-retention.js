...
export async function setRetentionPolicy(workspaceId, policy) {
  ...
  for (const key of Object.keys(DEFAULT_POLICY)) {
    if (key === "legalHold") {
      sanitized.legalHold = policy?.legalHold === true;
      continue;
    }
    if (key === "legalHoldReason") {
      sanitized.legalHoldReason =
        policy?.legalHoldReason == null ? null : String(policy.legalHoldReason).slice(0, 500);
      continue;
    }
    if (key === "legalHoldUntil") {
      if (policy?.legalHoldUntil == null || policy.legalHoldUntil === "") {
        sanitized.legalHoldUntil = null;
      } else if (Number.isNaN(Date.parse(policy.legalHoldUntil))) {
        throw new Error("legalHoldUntil must be a valid ISO date or null");
      } else {
        sanitized.legalHoldUntil = new Date(policy.legalHoldUntil).toISOString();
      }
      continue;
    }
...
export async function enforceRetention(workspaceId, userId = "anonymous") {
  ...
  const deleted = { conversations: 0, documents: 0, audit: 0, memory: 0, skippedLegalHold: false };

  if (policy.legalHold === true) {
    const until = policy.legalHoldUntil ? Date.parse(policy.legalHoldUntil) : null;
    if (until == null || until > Date.now()) {
      return { deleted: { ...deleted, skippedLegalHold: true }, legalHold: true };
    }
  }
...
/**
 * Phase 78.4: Enable or clear a legal hold that blocks retention enforcement.
 * @param {string} workspaceId
 * @param {{ enabled: boolean, reason?: string, until?: string|null }} opts
 */
export async function setLegalHold(workspaceId, opts = {}) {
  const current = await getRetentionPolicy(workspaceId);
  return setRetentionPolicy(workspaceId, {
    ...current,
    legalHold: opts.enabled === true,
    legalHoldReason: opts.enabled ? opts.reason || current.legalHoldReason || "legal_hold" : null,
    legalHoldUntil: opts.enabled ? opts.until ?? current.legalHoldUntil : null,
  });
}
