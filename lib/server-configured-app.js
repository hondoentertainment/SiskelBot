...
import { hasDurableStorage } from "./env-data-root.js";
...
// Production security: missing API_KEY leaves chat proxy open. Prefer failing closed on
// dedicated VMs via REQUIRE_PRODUCTION_API_KEY=1. Default is warn-only so serverless
// cold starts (and health checks) never brick when env is incomplete.
if (IS_PRODUCTION && !API_KEY) {
  const hint =
    "Set API_KEY (recommended), ALLOW_INSECURE_PRODUCTION=1 to acknowledge risk, or REQUIRE_PRODUCTION_API_KEY=1 to refuse boot.";
  if (process.env.ALLOW_INSECURE_PRODUCTION === "1") {
    console.warn(
      "[SECURITY] NODE_ENV=production but API_KEY is not set. /v1/chat/completions is publicly callable. " +
        "Continuing because ALLOW_INSECURE_PRODUCTION=1.",
    );
  } else if (process.env.REQUIRE_PRODUCTION_API_KEY === "1") {
    console.error("[SECURITY] NODE_ENV=production but API_KEY is not set. " + hint);
    process.exit(1);
  } else {
    console.error("[SECURITY] NODE_ENV=production but API_KEY is not set. /v1/chat/completions is publicly callable. " + hint);
  }
}
...
function validateStartupConfig() {
  ...
  if (IS_PRODUCTION && isLikelyVercelDeployment() && !hasDurableStorage()) {
    console.error(
      "[startup] Ephemeral storage on Vercel: set STORAGE_PATH (volume) or STORAGE_BACKEND=postgres with DATABASE_URL. " +
        "See docs/PRODUCTION.md.",
    );
    if (process.env.REQUIRE_DURABLE_STORAGE === "1") {
      process.exit(1);
    }
  }
}
...
// Normalize JSON Content-Type so body-parser accepts common client variants.
// Some SDKs send `application/json; charset=UTF-8` (mixed case / quoted), which
// can surface as "unsupported charset" during parse even though JSON is UTF-8 (RFC 8259).
function normalizeJsonRequestContentType(req, _res, next) {
  const ct = req.headers["content-type"];
  if (!ct || typeof ct !== "string") return next();
...
app.use(normalizeJsonRequestContentType);
...
// Prefer per-workspace buckets when the client sends workspace (body, query, or header).
function rateLimitWorkspaceSuffix(req) {
  const raw =
    req.body?.workspace ??
    req.body?.agentOptions?.workspace ??
    req.query?.workspace ??
  ...
  if (!s) return "";
  try {
    return `:ws:${storage.sanitizeWorkspace(s)}`;
  } catch {
    return ":ws:invalid";
  }
}
...
const perKeyChatRateLimiter =
  RATE_LIMIT_PER_KEY != null
    ? rateLimit({
        ...
        keyGenerator: (req) => `key:${req.apiKeyId || "unknown"}${rateLimitWorkspaceSuffix(req)}`,
        handler: (req, res) => {
...
const chatRateLimiter = rateLimit({
  ...
  legacyHeaders: false,
  keyGenerator: (req) => {
    const ws = rateLimitWorkspaceSuffix(req);
    if (isAuthConfigured() && req.userId && req.userId !== "anonymous") {
      return `user:${req.userId}${ws}`;
    }
    return `${req.ip || req.socket?.remoteAddress || "unknown"}${ws}`;
  },
  handler: (req, res) => {
...
const knowledgeIndexRateLimiter = rateLimit({
  ...
  legacyHeaders: false,
  keyGenerator: (req) =>
    `${req.ip || req.socket?.remoteAddress || "unknown"}${rateLimitWorkspaceSuffix(req)}`,
  handler: (req, res) => {
...
// Optional redirects for `/`:
// - `UI_MARKETING_ROOT=1` sends `/` to `/landing.html` (preserve query); `?app=1`
//   or `?app=true` skips to `/index.html` or `/app/chat` when `UI_DEFAULT_APP=1`.
// - Otherwise `UI_DEFAULT_APP=1` sends `/` to `/app/chat`. Both off: static `/`.
// Exported for tests (see tests/app-default-redirect.test.js).
installDefaultAppRedirect(app);
if (process.env.UI_MARKETING_ROOT === "1") {
  console.log("[ui] / → /landing.html (add ?app=1 to open chat) ENABLED via UI_MARKETING_ROOT=1");
}
...
/**
 * Install redirect for `GET /` (marketing landing or default app shortcuts).
 *
 * Precedence when `process.env.UI_MARKETING_ROOT === "1"`:
 * redirects `/` to `/landing.html`, unless query `app=1` or `app=true`,
 * then removes `app` and sends to `/index.html`, or `/app/chat` when
 * `process.env.UI_DEFAULT_APP === "1"`.
 *
 * Otherwise, when `UI_DEFAULT_APP === "1"`, redirect `/` to `/app/chat`.
 * When unset, no-op (`/` served by express.static client mount).
 *
 * Exported for tests (tests/app-default-redirect.test.js).
 */
export function installDefaultAppRedirect(targetApp, env = process.env) {
  // Marketing landing at `/` (opt-in). `?app=1` skips to the chat app.
  // Precedence over UI_DEFAULT_APP when both are set.
