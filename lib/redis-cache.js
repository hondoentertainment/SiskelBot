...
export function createRedisCache(options = {}) {
   ...
   * Returns the number of distinct logical keys removed
   * (a key present in both Redis and the memory fallback counts once).
   */
      ...
      try {
        ...
        do {
          ...
          if (parsed.keys.length > 0) {
            ...
            for (const k of parsed.keys) removed.add(k);
          }
        } while (cursor !== 0);
      } catch (err) {
