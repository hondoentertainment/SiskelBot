...
/**
 * List enabled scheduled agents whose nextRun is due (or overdue).
 * @param {string} [workspaceId] — if omitted, scans ADVICE_BRIEF_WORKSPACES / INITIATIVE_WORKSPACES / default
 * @returns {Promise<Array<{ workspaceId: string, agent: object }>>}
 */
export async function listDueScheduledAgents(workspaceId) {
  const workspaces = workspaceId
    ? [sanitizeWorkspace(workspaceId)]
    : (process.env.SCHEDULED_AGENT_WORKSPACES || process.env.INITIATIVE_WORKSPACES || "default")
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean);
  const now = Date.now();
  ...
  for (const ws of workspaces) {
    ...
    for (const agent of agents) {
      ...
      if (!agent.nextRun) continue;
      const t = Date.parse(agent.nextRun);
      if (!Number.isNaN(t) && t <= now) due.push({ workspaceId: ws, agent });
    }
  }
  return due;
}
...
/**
 * Run all due scheduled agents (best-effort per agent).
 * @param {{ backendFetch?: Function, buildProxyConfig?: Function, workspaceId?: string }} [opts]
 */
export async function runDueScheduledAgents(opts = {}) {
  const due = await listDueScheduledAgents(opts.workspaceId);
  const results = [];
  for (const { workspaceId, agent } of due) {
    try {
      // Refresh morning-brief prompt with current journal before each run
      if (agent.name === "Advice morning brief") {
        try {
          const { buildMorningBrief } = await import("./advice-brief.js");
          const brief = await buildMorningBrief(opts.userId || "anonymous", workspaceId);
          await updateScheduledAgent(workspaceId, agent.id, { prompt: brief.promptForAgent });
        } catch {
          /* best-effort refresh */
        }
      }
      const r = await runScheduledAgent(workspaceId, agent.id, opts);
      results.push({ workspaceId, agentId: agent.id, ok: !r.error, ...r });
    } catch (err) {
      results.push({ workspaceId, agentId: agent.id, ok: false, error: err.message });
    }
  }
