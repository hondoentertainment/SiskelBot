...
import { writeChainedEntry } from "./audit-chain.js";
...
registerAction("webhook", async function webhookHandler(payload, _ctx = {}) {
...
/**
 * Append an audit log entry. Persists via json-path-store (Postgres / SQLite / file).
  ...
  try {
    ...
    if (workspaceId) {
      const chainEntry = {
        ...baseEntry,
        event: entry.action,
        createdAt: baseEntry.timestamp,
        metadata: entry.payload ?? null,
      };
      await writeChainedEntry(workspaceId, chainEntry);
      return;
    }

    await withPathLock(AUDIT_LOG_PATH, async () => {
      const loaded = await readJsonPath(AUDIT_LOG_PATH, []);
      let entries = Array.isArray(loaded) ? loaded : [];
      entries.push(baseEntry);
      entries = trimAuditEntries(entries, AUDIT_MAX_ENTRIES, AUDIT_RETENTION_DAYS);
      await writeJsonPath(AUDIT_LOG_PATH, entries);
    });
  } catch (e) {
