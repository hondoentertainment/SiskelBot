...
export async function executeStep(step, ctx = {}) {
  ...
  // Phase 69: span per recipe step execution
  return withSpan("recipe.step", {
    "recipe.step_name": step.name || action,
    "recipe.action_type": action,
  }, async (span) => {
    const result = await handler(payload, ctx);
    span.setAttributes({ "recipe.success": result.ok !== false });
    return result;
  });
}
