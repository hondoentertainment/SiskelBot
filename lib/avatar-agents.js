...
function normalizeVoice(voice, { partial = false } = {}) {
  ...
  if (partial) {
    const out = {};
    if (voice.provider !== undefined) out.provider = String(voice.provider);
    if (voice.voiceId !== undefined) out.voiceId = String(voice.voiceId);
    if (voice.pitch !== undefined && Number.isFinite(voice.pitch)) out.pitch = Number(voice.pitch);
    if (voice.rate !== undefined && Number.isFinite(voice.rate)) out.rate = Number(voice.rate);
    if (voice.volume !== undefined && Number.isFinite(voice.volume)) out.volume = Number(voice.volume);
    return out;
  }
  return {
...
export async function updateAvatarConfig(avatarId, updates = {}) {
  ...
  await withPathLock(configsPath(), async () => {
    ...
    if (updates.voice !== undefined) next.voice = { ...current.voice, ...normalizeVoice(updates.voice, { partial: true }) };
...
function normalizeState(state, base = {}) {
  ...
  // Field was explicitly provided when the key is present on the update object.
  let mood;
  if ("mood" in s) {
    mood = typeof s.mood === "string" && VALID_MOODS.has(s.mood) ? s.mood : "neutral";
  } else {
  ...
  if ("animation" in s) {
    animation =
      typeof s.animation === "string" && VALID_ANIMATIONS.has(s.animation) ? s.animation : "idle";
  } else {
    animation = base.animation || "idle";
  }
  const lookAt = s.lookAt !== undefined ? normalizeVec3(s.lookAt, base.lookAt || [0, 0, 0]) : base.lookAt || [0, 0, 0];
  const speaking = "speaking" in s ? !!s.speaking : !!base.speaking;
  const gesture = "gesture" in s ? (typeof s.gesture === "string" ? s.gesture : null) : (base.gesture || null);
