 ...
 *
 * Wave-2: When AGENT_PLAN_RAG=1 also, past successful trajectories matching
 * the user request by keyword are injected into the planner prompt as
 * few-shot examples.
 */
import { planRetrievalEnabled, findSimilarTrajectories, formatExamplesForPrompt } from "./agent-plan-retrieval.js";
...
export async function buildUpfrontPlan({
  ...
  // Wave-2: Pull similar past successful trajectories and inject as few-shot
  // examples. No-op when AGENT_PLAN_RAG is off or there are no examples.
  let examplesBlock = "";
  if (planRetrievalEnabled()) {
    try {
      const examples = await findSimilarTrajectories({ userMessage, workspace });
      examplesBlock = formatExamplesForPrompt(examples);
    } catch { /* best-effort */ }
  }

  const systemContent = examplesBlock
    ? `${PLANNING_PROMPT.replace(`${DEFAULT_MAX_STEPS}`, String(maxSteps))}\n\n${examplesBlock}`
    : PLANNING_PROMPT.replace(`${DEFAULT_MAX_STEPS}`, String(maxSteps));
