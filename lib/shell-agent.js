...
export async function executeCommand(command, options = {}) {
  ...
  const started = now();
  const portable = runPortableCommand(program, args, { cwd, command, started });
...
function runPortableCommand(program, args, { cwd, command, started }) {
  const name = String(program || "").toLowerCase();
  try {
    if (name === "echo") {
      return commandResult({ command, started, stdout: `${args.join(" ")}\n` });
    }
    if (name === "ls") {
      const target = resolveCommandPath(args[0] || ".", cwd);
      const entries = readdirSync(target).join("\n");
      return commandResult({ command, started, stdout: entries ? `${entries}\n` : "" });
    }
    if (name === "cat") {
      if (args.length === 0) return commandResult({ command, started, stderr: "cat: missing file operand\n", exitCode: 1 });
      const stdout = args.map((arg) => readFileSync(resolveCommandPath(arg, cwd), "utf8")).join("");
      return commandResult({ command, started, stdout });
    }
    if (name === "grep") {
      if (args.length < 2) return commandResult({ command, started, stderr: "grep: missing pattern or file\n", exitCode: 1 });
      const [pattern, ...files] = args;
      const needle = String(pattern);
      const matches = [];
      for (const file of files) {
        const abs = resolveCommandPath(file, cwd);
        const lines = readFileSync(abs, "utf8").split(/\r?\n/);
        for (const line of lines) {
          if (line.includes(needle)) matches.push(line);
        }
      }
      return commandResult({ command, started, stdout: matches.length ? `${matches.join("\n")}\n` : "", exitCode: matches.length ? 0 : 1 });
    }
    if (name === "find") {
      const target = resolveCommandPath(args[0] || ".", cwd);
      const paths = [];
      walkPaths(target, paths);
      return commandResult({ command, started, stdout: paths.length ? `${paths.join("\n")}\n` : "" });
    }
  } catch (err) {
...
function commandResult({ command, started, stdout = "", stderr = "", exitCode = 0 }) {
  return {
    stdout,
    stderr,
    exitCode,
...
function resolveCommandPath(target, cwd) {
  return resolve(cwd, String(target || "."));
}

function walkPaths(target, out) {
  out.push(target);
  const st = statSync(target);
  if (!st.isDirectory()) return;
  for (const entry of readdirSync(target)) {
    walkPaths(join(target, entry), out);
  }
}
