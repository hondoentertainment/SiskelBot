...
import { getPlan, setPlan } from "./plans.js";
...
export function createBillingManager() {
  return {
    ...
    async recordUsage(workspaceId, tokens, model) {
      ...
      // Optional usage-based billing: report to the Stripe meter (no-op
      // unless STRIPE_METER_EVENT_NAME is configured; never throws). Fired
      // detached so billing I/O can never stall the chat response path.
      try {
        const { reportMeteredUsage } = await import("./stripe-metering.js");
        reportMeteredUsage(record.workspaceId, totalTokens).catch(() => {});
...
let _stripe = null;
let _stripeLoad = null;

async function getStripe() {
  if (!process.env.STRIPE_SECRET_KEY) return null;
  if (!_stripe) {
    if (!_stripeLoad) {
      _stripeLoad = import("stripe")
        .then((mod) => mod.default)
        .catch(() => null);
    }
    const Stripe = await _stripeLoad;
...
/**
 * Persist plan tier into workspace storage so tenant quotas (requests/min, etc.)
 * match the billing plan (see lib/tenant-quotas.js `billing:<workspaceId>`).
 * @param {{ add?: (type: string, item: object, workspace?: string, merge?: boolean, userId?: string) => Promise<unknown> }} storage
 * @param {string} workspaceId
 * @param {"free"|"pro"|"enterprise"} planId
 */
export async function writeTenantBillingPlanToStorage(storage, workspaceId, planId) {
  if (!storage || typeof storage.add !== "function") return;
  const ws = String(workspaceId || "default");
  const tier = planId === "pro" || planId === "enterprise" || planId === "free" ? planId : "free";
  try {
    await storage.add(`billing:${ws}`, { id: "default", plan: tier }, "default", true, "anonymous");
  } catch (e) {
    console.warn("[billing] tenant billing storage sync failed:", e?.message || e);
  }
}

/**
 * After `billing-subscriptions/<workspace>.json` is updated, sync workspace-plans.json
 * and the tenant `billing:*` row so quotas, GET /billing/plan, and Stripe stay aligned.
 * @param {string} workspaceId
 * @param {{ add?: Function }} [storage]
 */
export async function syncWorkspaceBillingFromStripeFile(workspaceId, storage) {
  const ws = String(workspaceId || "").trim();
  if (!ws) return;
  const data = await readJsonPath(subscriptionPath(ws), {});
  const status = String(data.status || "active");
  const rawPlan = String(data.plan || "free");
  const paidTier =
    (status === "active" || status === "past_due") && (rawPlan === "pro" || rawPlan === "enterprise");
  const target = paidTier ? rawPlan : "free";
  const r = await setPlan(ws, target);
  if (!r.ok) console.warn("[billing] setPlan post-webhook:", ws, r.error);
  await writeTenantBillingPlanToStorage(storage, ws, /** @type {"free"|"pro"|"enterprise"} */ (target));
  // Index the workspace for revenue analytics (best-effort, never throws).
  try {
    const { registerBillingWorkspace } = await import("./revenue-analytics.js");
    await registerBillingWorkspace(ws);
 ...
 * @param {{ stripeCustomerId: string, returnUrl: string }} params
 * @returns {Promise<{ url: string } | { url: null }>}
 */
 ...
 * @param {{ storage?: { add?: Function } }} [ctx]
 * @returns {Promise<{ handled: boolean, event: string }>}
 */
export async function handleWebhookEvent(rawBody, signature, ctx = {}) {
  const { storage } = ctx || {};
  if (!process.env.STRIPE_SECRET_KEY) {
    return { handled: false, event: "stripe_disabled" };
  }
  ...
  const stripe = await getStripe();
  if (!stripe) throw new Error("Stripe SDK is not available");
  ...
  switch (event.type) {
    case "checkout.session.completed": {
      ...
      if (workspaceId && planId) {
        ...
        await syncWorkspaceBillingFromStripeFile(workspaceId, storage);
      }
      break;
    }
