 ...
 *
 * Token-aware trimming: trimToTokenBudget() uses token estimates to drop
 * oldest tool rounds before the conversation exceeds the model's context window.
 */

import { estimateMessagesTokens, getModelContextWindow } from "./token-counter.js";
...
/**
 * Trim messages to fit within a token budget by removing oldest tool rounds first,
 * then oldest non-system messages if still over budget.
 *
 * @param {object[]} messages - Chat messages array
 * @param {{ model?: string; maxContextTokens?: number; reserveTokens?: number }} [opts]
 * @returns {{ trimmedMessages: object[]; tokensBefore: number; tokensAfter: number; roundsRemoved: number }}
 */
export function trimToTokenBudget(messages, opts) {
  ...
  const maxContextTokens = opts?.maxContextTokens ?? Math.floor(contextWindow * 0.85);
  const reserveTokens = opts?.reserveTokens ?? 2000;
  const budget = Math.max(0, maxContextTokens - reserveTokens);
  ...
  // Phase 1: remove oldest tool rounds until within budget
  const segments = segmentMessages(messages);
  let roundsRemoved = 0;

  while (estimateMessagesTokens(flattenSegments(segments)) > budget) {
    const idx = segments.findIndex((s) => s.type === "tool_round");
    if (idx < 0) break;
    segments.splice(idx, 1);
    roundsRemoved++;
  ...
  let trimmed = flattenSegments(segments);

  // Phase 2: if still over budget after removing all tool rounds, drop oldest
  // non-system messages (preserve system messages and the latest user message)
  if (estimateMessagesTokens(trimmed) > budget && trimmed.length > 1) {
    // Find the last user message index to preserve it
    let lastUserIdx = -1;
    ...
    // Remove oldest non-system, non-last-user messages until within budget
    while (estimateMessagesTokens(trimmed) > budget && trimmed.length > 1) {
      let removed = false;
      for (let i = 0; i < trimmed.length; i++) {
        if (trimmed[i]?.role === "system") continue;
        if (i === lastUserIdx) continue;
        trimmed.splice(i, 1);
        // Recalculate lastUserIdx after removal
        if (lastUserIdx > i) lastUserIdx--;
        removed = true;
        break;
      }
      if (!removed) break; // only system + last user left
    }
  }

  const tokensAfter = estimateMessagesTokens(trimmed);
  return { trimmedMessages: trimmed, tokensBefore, tokensAfter, roundsRemoved };
}
