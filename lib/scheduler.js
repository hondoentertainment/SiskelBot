...
import { runInitiativeCycle, formatProposalMessage, formatProposalBlocks } from "./initiative-engine.js";
import { sendSlackMessage, isSlackConfigured } from "./slack-bot.js";
import { runDueScheduledAgents } from "./scheduled-agents.js";
...
async function surfaceProposalToSlack(proposal) {
  ...
  try {
    await sendSlackMessage(INITIATIVE_SLACK_CHANNEL || undefined, formatProposalMessage(proposal), {
      blocks: formatProposalBlocks(proposal),
    });
  } catch (e) {
...
export async function runDueJobs(workspace = "default") {
  ...
  // Due cron agents (including Advice morning brief) — independent of recipe execution.
  let scheduledAgents = { ran: 0, total: 0 };
  try {
    scheduledAgents = await runDueScheduledAgents({ workspaceId: workspace });
  } catch (e) {
    log.warn("Scheduled agents tick error", { error: e.message });
  }

  if (!ALLOW_EXECUTION) {
    log.info("Skipped: ALLOW_RECIPE_STEP_EXECUTION != 1");
    return { ran: 0, skipped: true, rotations, warehouseExports, groupSync, scheduledAgents };
  }
  ...
  return { ran, rotations, warehouseExports, groupSync, scheduledAgents };
}
...
export async function runDueJobsVercel(workspace = "default") {
  ...
  let initiative = { created: 0 };
  if (ENABLE_INITIATIVE_ENGINE) {
    try {
      initiative = await runInitiativeCycles();
    } catch (e) {
      log.warn("Initiative tick error", { error: e.message });
    }
  }

  if (!ALLOW_EXECUTION) return { ran: 0, skipped: true, scheduledAgents, initiative };
  let ran = 0;
  const all = await schedules.list(workspace);
  const items = all.filter((s) => s.enabled && s.cron);
  for (const sched of items) {
  ...
  return { ran, scheduledAgents, initiative };
}
...
export async function start() {
  ...
  // Due scheduled agents (cron nextRun) every minute — leader-only.
  try {
    cron.schedule(
      "* * * * *",
      async () => {
        const stillLeader = await leader.isLeader();
        if (!stillLeader) return;
        await runDueScheduledAgents().catch((e) => log.warn("Scheduled agents tick error", { error: e.message }));
      },
      { scheduled: true }
    );
    log.info("Scheduled agents minute tick enabled");
  } catch (e) {
    log.warn("Failed to schedule agent minute tick", { error: e.message });
  }
}
