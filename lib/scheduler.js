...
export async function start() {
  ...
  if (!acquired) {
    ...
    if (!isLead) {
      console.log("[scheduler] Not leader — skipping scheduled recipe start (another instance is leader)");
      return;
    }
  }
  console.log("[scheduler] This instance is leader — starting scheduled recipes");

  // Listen for leadership changes
  leader.onLeaderChange((isLeader) => {
    if (isLeader) {
      console.log("[scheduler] Acquired leadership — cron tasks will execute");
    } else {
      console.log("[scheduler] Lost leadership — cron tasks will be skipped");
    }
  });
    ...
    try {
      ...
      const task = cron.schedule(
        ...
        async () => {
          ...
          if (!stillLeader) {
            console.log("[scheduler] Skipping tick — not leader");
            return;
          }
          const recipe = await storage.get("recipes", recipeId, workspace);
