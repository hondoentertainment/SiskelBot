 ...
 *
 * When `taskText` is supplied, the hint is restricted to tools whose names
 * share at least one keyword with the task (e.g. "search the docs" -> only
 * search_* and list_context tools get cited). Falls back to the full
 * ranking when no tool names match the task keywords.
 *
 ...
 * @param {number} [opts.avoidBelow=0.3]
 * @param {number} [opts.preferAbove=0.8]
 * @param {number} [opts.maxListed=5]
 * @param {string} [opts.taskText] - user message or query to match against tool names
...
export async function buildReliabilityHint(workspace, opts = {}) {
  ...
  const taskText = typeof opts.taskText === "string" ? opts.taskText : "";

  const stats = await loadToolOutcomes(workspace);
  if (!stats.tools || Object.keys(stats.tools).length === 0) return "";

  const rankedAll = rankByReliability(stats, { minSamples });
  if (rankedAll.length === 0) return "";

  // If a task was supplied, restrict to tools whose names match the task
  // keywords. Fall back to the full ranking when nothing matches (avoids
  // silently dropping the hint for short task strings).
  let ranked = rankedAll;
  if (taskText) {
    const taskTokens = extractTaskKeywords(taskText);
    if (taskTokens.size > 0) {
      const filtered = rankedAll.filter((r) => toolMatchesTask(r.tool, taskTokens));
  ...
  const header = taskText && ranked !== rankedAll
    ? "Task-relevant tool reliability in this workspace (from prior runs):"
    : "Tool reliability in this workspace (from prior runs):";
  const parts = [header];
  if (prefer.length > 0) {
    parts.push(
      `High success: ${prefer
...
/**
 * Extract content keywords from task text for tool-name matching.
 * Exported for tests and downstream keyword-based tool routing.
 * @param {string} text
 * @returns {Set<string>}
 */
export function extractTaskKeywords(text) {
  ...
  for (const tok of matches) {
    ...
    out.add(tok);
    // Crude stemmer: drop trailing "s" / "ing" / "ed" so "searches" matches
    // "search" in a tool name like search_context.
    if (tok.endsWith("ing") && tok.length > 5) out.add(tok.slice(0, -3));
    else if (tok.endsWith("ed") && tok.length > 4) out.add(tok.slice(0, -2));
    else if (tok.endsWith("s") && tok.length > 3) out.add(tok.slice(0, -1));
  }
  return out;
}

/**
 * Does this tool name share a token with the task keywords?
 * @param {string} toolName
 * @param {Set<string>} taskTokens
 * @returns {boolean}
 */
export function toolMatchesTask(toolName, taskTokens) {
  if (!toolName || !taskTokens || taskTokens.size === 0) return false;
  const parts = toolName.toLowerCase().split(/[_\-\s]+/).filter(Boolean);
  for (const part of parts) {
    if (taskTokens.has(part)) return true;
    // Also check task-token prefix of the tool-name part (e.g. "search" in "semantic")
    for (const t of taskTokens) {
      if (t.length >= 4 && part.startsWith(t)) return true;
      if (part.length >= 4 && t.startsWith(part)) return true;
    }
  }
  return false;
}
...
const TASK_STOPWORDS = new Set([
  ...
  "please", "help", "tell", "show", "give", "make", "about", "into", "onto",
  "will", "would", "should", "could", "have", "has", "had", "does", "did",
  "are", "was", "were", "been", "being", "not", "but", "out", "all", "any",
  "get", "just", "now", "here", "there", "then", "than", "too",
]);
