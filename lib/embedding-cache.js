...
export function createEmbeddingCache(options) {
  ...
  async function load(filePath) {
    ...
    try {
      ...
      throw new Error(`embedding-cache: invalid JSON in ${filePath}: ${e.message}`, { cause: e });
    }
...
/**
 * Snapshot of the global embedding cache counters for Prometheus export.
 * Single source of truth for embedding cache observability.
 * @returns {{
 *   hits: number,
 *   misses: number,
...
export function getCacheStats() {
  const s = globalEmbeddingCache.stats();
  return {
    hits: s.hits,
    misses: s.misses,
    evictions: {
      lru: s.evictions?.lru || 0,
      ttl: s.evictions?.ttl || 0,
      manual: s.evictions?.manual || 0,
    },
    size: s.size,
    maxSize: s.maxSize,
